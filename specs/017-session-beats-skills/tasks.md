...
- [X] T018 [US6] Create `.agents/skills/spell-design/SKILL.md`. Primary for write, edit, or create of a spell page. Completes when narration, classification, and a runnable 2024 effect block are filled. Discovery and Lore when the spell needs placement or history. Design-impact: designated writer, Codex CLI at ChatGPT 5.5 medium after Claude Code usage-limit deferral.
...
- [X] T031 [US7] Create `.agents/skills/faction-design/SKILL.md` (Codex CLI gpt-5.5 medium). Primary for write, edit, or create of a faction page. Completes when FR-028 jobs are filled. Quote: "Current Turn named with no roll at create". Agenda clock lives on the faction page. `hot.md` MAY point; MUST NOT store a second clock. State what to write and when the page is done. Design-impact: designated writer, Codex CLI at ChatGPT 5.5 medium after Claude Code usage-limit deferral.
- [X] T032 [US7] Delete `.agents/skills/faction-prep/` after T031. Retarget remaining live callers in `.agents/skills/` and `AGENTS.md` to `faction-design`. Do not keep a stub. Do not rewrite `legacy/`.
...
- [X] T036 [US8] Create `.agents/skills/lore-design/SKILL.md` (Codex CLI gpt-5.5 medium). Primary for write, edit, or create of a lore page. Completes when FR-035 jobs are filled. Unrelated truths split into linked notes. MUST NOT mark lore `canon` until players interact or witness. Canon Log omitted until then. MUST NOT invent table history. Design-impact: designated writer, Codex CLI at ChatGPT 5.5 medium after Claude Code usage-limit deferral.
...
- [X] T040 [US9] Redesign `.agents/skills/narrative-islands/SKILL.md` (Codex CLI gpt-5.5 medium) to fill `wiki/templates/quest.md` as primary for write/edit/create of a quest page. Owns later updates including World in motion, portents, rolls, Situation, status, and the Quest log. MUST NOT mint `type: front` or `type: encounter`. MUST NOT add a `quest-design` skill. Keep T020 typed-beat retarget: chart → `session-beats`, typed beat → matching type skill. Design-impact: designated writer, Codex CLI at ChatGPT 5.5 medium after Claude Code usage-limit deferral.
...
- [X] T024 Run `specs/017-session-beats-skills/quickstart.md` steps 1–10 against `specs/017-session-beats-skills/contracts/beat-skill-routing.md` and `specs/017-session-beats-skills/contracts/wiki-kind-pages.md`. Result: jobs 1–15 and 16–20, wiki-kind jobs 21–31, isolation, owners, and dispatch evidence hold. Session-prep kind templates `wiki/templates/{hook,development,cliffhanger,climax,resolution,session-plan}.md` are not in this task list and are absent; steps 2/6 typed-draft copy-start cannot be demonstrated until those files exist.
- [X] T025 Confirm this change set does not rewrite bodies of `wiki/_raw/Session-11-*.md`, does not edit `.agents/skills/writing-beats/SKILL.md`, does not copy routing tables into `.omp/AGENTS.md`, leaves 0 skills that contain both the full Beat Chart and all five type-card catalogs, does not add a `quest-design` skill, has no `.agents/skills/faction-prep/`, does not remap `lore`→`item`, and does not file new durable situations as `type: front` or `type: encounter`
- [X] T026 Confirm designated-writer jobs T003, T005–T009, T017 used `claude-opus-4-6 --effort medium`; T018, T031, T036, T040, T043, T046 used Codex CLI gpt-5.5 medium with prompts that name deliverables plus a completion test after Claude Code usage-limit deferral (owner instruction this session); `AGENTS.md` and templates were session-agent work
...
**Purpose**: Close the session-prep kind gap. Drafts are in `wiki/_raw/`. Do not rewrite Session 11 files.

- [X] T049 Install `wiki/templates/hook.md`, `wiki/templates/development.md`, `wiki/templates/cliffhanger.md`, `wiki/templates/climax.md`, `wiki/templates/resolution.md`, and `wiki/templates/session-plan.md` from `wiki/_raw/hook-template.md`, `wiki/_raw/development-template.md`, `wiki/_raw/cliffhanger-template.md`, `wiki/_raw/climax-template.md`, `wiki/_raw/resolution-template.md`, and `wiki/_raw/session-beat-template.md` per FR-054 (missing). Rewrite identity per `specs/017-session-beats-skills/contracts/session-prep-pages.md`: `type: session-prep`; `kind` matching the page (`hook` \| `development` \| `cliffhanger` \| `climax` \| `resolution` \| `session-plan`); `category: journal`; drop illegal draft keys (`type: session-beat` / `type: session-plan`, `beat:`, `intensity`, `pacing`, `card` / `hook_kind` / `development_type` / `climax_kind`, minute/order/sequence/duration fields, `location`, previous/next aliases, `participants`, `status`, `date`, `planned_length`, `previous_session`, `next_session`). Keep draft jobs. Shape `col` / `col-md` where a dashboard pair shares the scan. Strip HTML design-basis comments and citation crumbs. Session-plan comment that treats alternation as optional MUST NOT ship. MUST NOT add `type: beat`, `type: session-beat`, or `type: session-plan`. Session agent.
- [X] T050 Redesign `.agents/skills/session-beats/SKILL.md` and `.agents/skills/session-beats/references/session-skeleton.md` so composition fills `wiki/templates/session-plan.md` (compass, beat map, floating beats, pressure, PC touchpoints, links to typed beat pages) with `type: session-prep` `kind: session-plan` per FR-002 (contradicts). Stop filing Session 11-00 spine jobs (length, tone, prize, opposition, numbered skeleton). Stop composing live beats with `run-guide` in Session 11 cockpit shape. Skeleton MUST NOT teach Session 11-00 as the filed page. Keep chart rules, polarity, recompute, typed-fill seams. Design-impact: designated writer, `claude-opus-4-6 --effort medium`.
- [X] T051 In `.agents/skills/run-guide/SKILL.md`, stop requiring Session 11 cockpit job order as the live-beat pass test per FR-015 (contradicts). Live-beat assembly MUST NOT rewrite a typed beat into a Session 11 cockpit. Keep four-pass owners (mechanics, copy-writer, theatre of the mind, ready check). Session 11 files remain scan-quality evidence only. Session agent.
- [X] T052 In `.agents/skills/hook-beats/SKILL.md`, `.agents/skills/development-beats/SKILL.md`, `.agents/skills/cliffhanger-beats/SKILL.md`, `.agents/skills/climax-beats/SKILL.md`, and `.agents/skills/resolution-beats/SKILL.md`, copy-start from `wiki/templates/{hook,development,cliffhanger,climax,resolution}.md` and file `type: session-prep` with matching `kind` per FR-013 (missing). Keep that type's draft jobs. Do not absorb cockpit job order. Session agent.
- [X] T053 In `wiki/AGENTS.md`, add `kind` `hook` \| `development` \| `cliffhanger` \| `climax` \| `resolution` \| `session-plan` on `type: session-prep` and Layout jobs for those kinds matching `specs/017-session-beats-skills/contracts/session-prep-pages.md` per FR-055 (missing). Stop using `wiki/templates/session-prep.md` as copy-start for new beats or plans. Replace Session beat / Session spine Layout rows. Filing path `Session-<n>-00-<Title>.md` / `Session-<n>-<BB>-<Label>.md` stays. Do not adopt draft `{{title}} - B01 - Strong Start` filenames. Session agent.
