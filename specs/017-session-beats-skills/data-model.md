...
| Filed spine jobs (007) | Wiki write before accept |
...
| Field | Rule |
|---|---|
| Kind | `plan-session` \| `write-beat` \| `edit-beat` \| `fill-beat` |
| Type | Empty on `plan-session`; else one of the five |
| Primary skill | Composition if `plan-session`; else that type skill |
...
- type skill → existing crafts (theatre of the mind, encounter-prep, traps-trials, place, monster) by handoff, not absorption
