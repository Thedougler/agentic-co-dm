...
- [ ] T030 [US2] Add subprocess tests in `tests/test_creative_lint_cli.py` for `task`, `file`, `corpus`, `changed`, and `rule`, including JSON schema, human rendering, severity filtering, unknown bundle/rule errors, and exit codes.
...
- [ ] T036 [US2] Implement human-readable rendering and severity filtering in `tools/creative_lint/cli_output.py` to match `contracts/cli-contract.md` while keeping JSON machine-readable.
