...
- Off-the-shelf linting tools are preferred over custom implementations wherever feasible. Vale handles prose-pattern rules through the package set declared in the repository's `.vale.ini` (`ai-tells`, `proselint`, `Readability`, and `Harper`); markdownlint-cli2 handles structural markdown rules. Together they subsume the custom Python lint scripts (`lint-obsidian-markdown`, `lint-literal-newlines`). Custom Python evaluators are reserved for cross-page symbolic checks that no off-the-shelf tool supports.
