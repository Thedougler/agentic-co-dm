...
- Q: Which token count should be authoritative when local and provider-reported counts differ? → A: Use one declared authoritative token method per measurement family; compare token measurements only within the same family.
- Q: Where should normal sitting traces be stored and how long should they be retained? → A: Store redacted telemetry locally with bounded retention; sanitized evaluation fixtures and pinned baselines may be committed.
...
- Q: Who should own the efficiency policy and its safety thresholds? → A: Maintainers own the versioned efficiency policy; agents may read and propose changes but may not silently weaken thresholds, gates, or autonomy boundaries.
- Q: How should retrieval fallback behavior be recorded when a higher-precedence collection is silent or fails? → A: Log every attempted collection and fallback, preserve wiki → shattered-sea → legacy precedence, and never let fallback material override current accepted canon.
...
- Q: What should happen before the separate measurement-governance change is accepted? → A: Collect and report redacted telemetry, but block cross-family comparisons and promotion; affected records carry an explicit measurement-gap reason and are excluded from complete comparison samples.
...
- Q: Should concrete implementation constraints remain in the specification or move to the plan and contracts? → A: Keep observable behavior and acceptance outcomes in the specification; move concrete paths, storage formats, and tool choices to the plan and contracts.
...
5. **Given** a normal technical tooling, automation, storage, retrieval, or test request, **When** the request is classified, **Then** it is `engineering` and normal engineering SDD semantics remain in force.
...
2. **Given** an entity with no matching owner after targeted identity resolution, **When** a new owner is genuinely required, **Then** the specification identifies the required artifact, its existing repository kind and canonical owner convention, and its provenance obligations without introducing opaque IDs.
...
6. **Given** a safe deterministic maintenance action such as validation, metadata maintenance, retrieval refresh, or link repair, **When** it does not change campaign facts, **Then** it is not blocked by a needless human approval gate.
...
- **Scenario D — Routine NPC**: One ordinary NPC routes to the existing NPC skill/template process without generating full SDD feature artifacts or an engineering task graph.
- **Scenario E — Existing entity collision**: An apparent new entity that matches an existing title, alias, canonical identity signal, or duplicate candidate reuses the owner page.
...
- The separate measurement-governance change is not yet accepted: redacted trace collection and reporting MAY proceed, but cross-family comparisons and promotion MUST be blocked; affected records carry a measurement-gap reason and are not complete comparison samples.
...
- **FR-014**: Before proposing a new canonical entity, the process MUST resolve existing titles, aliases, canonical identity signals, and duplicate candidates using existing repository identity mechanisms.
...
- **FR-028**: Context strategy MUST prefer minimum sufficient authoritative retrieval, owner-page retrieval, targeted relationships, current state, invoked rules, progressive disclosure, machine-readable discovery, and environment-derived truth. A fetched page MUST count as materially useful only when accepted Work explicitly cites or links it, or an independent evaluator confirms that it supplied a required fact, rule, or safety constraint; fetched-but-unused pages MUST count as unnecessary retrieval. The retrieval layer MUST record every attempted collection and fallback while preserving wiki → shattered-sea → legacy precedence; fallback material MUST NOT override current accepted canon.



- **FR-030**: Any claimed token or context improvement MUST be measured with the authoritative method declared by the efficiency policy across the whole useful trajectory from the user request through final Work, including loaded instructions and context, retrieval, tool calls, failed calls, retries, and model input/output; idle time and unrelated activity MUST be excluded. Comparisons MUST remain within the same measurement family unless a separately specified normalization method exists. Comparisons MUST preserve narrative craft, mechanics, specificity, canon fidelity, playability, agency, and DM usefulness. Normal sitting telemetry MUST be redacted, locally retained under the policy, and excluded from committed sensitive content; sanitized evaluation fixtures and pinned baselines MAY be committed.
...
- **FR-039**: Efficiency policy MUST be owned by maintainers in a versioned policy artifact. The policy MUST define measurement rules, sample requirements, non-inferiority requirements, autonomy classes, canary rules, and rollback thresholds. Agents MAY read and propose policy changes but MUST NOT silently weaken thresholds, quality gates, or autonomy boundaries.
...
- **FR-041**: Redacted trajectory telemetry MUST be collected for every `prep` and `wrapup` sitting by default. An explicit policy setting MAY disable collection, but the resulting sitting MUST record an observable measurement gap and MUST NOT be treated as a complete comparison sample. Before the separate measurement-governance change is accepted, collection and redacted reporting MAY proceed, but cross-family comparisons and promotion MUST be blocked; affected records MUST carry an explicit measurement-gap reason and MUST NOT count as complete comparison samples.
...
- **Context cost**: Measured context and token use across the whole useful trajectory of comparable work, using the authoritative method declared by the efficiency policy, from user request through final Work, including loaded context, retrieval, tools, failures, retries, and model input/output, while excluding idle and unrelated activity. Measurements from different measurement families are not directly comparable. Normal telemetry is redacted, locally retained under policy, and committed fixtures and baselines contain no raw sensitive content.
...
- **SC-021**: In a retrieval fallback validation run with silent and failed higher-precedence collections, 100% record every attempted collection and fallback, preserve wiki → shattered-sea → legacy precedence, and prevent fallback material from overriding accepted canon.
...
- **SC-023**: In a telemetry-coverage validation run containing enabled, explicitly disabled, and pre-governance sittings, 100% of enabled prep and wrapup sittings produce telemetry; every disabled sitting records a measurement gap; and every pre-governance sitting may report redacted telemetry but has cross-family comparison and promotion blocked, carries a measurement-gap reason, and is excluded from complete comparison samples.
...
- Existing identity resolution uses canonical titles, aliases, repository identity conventions, and duplicate detection; opaque IDs are unnecessary by default.
- The repository's Spec Kit integration and current catalog are discovery sources. Community presets may be downloaded as pinned, reviewable candidates and selectively adapted after inspection, but remain untrusted references rather than live dependencies or authorities.
...
- Context/token comparisons use the authoritative method declared by the efficiency policy and compare same-kind work within that measurement family; lower cost is not a success when quality or completeness falls.
- A separate measurement-governance change is required before cross-family comparisons and promotion can activate. Until that change is accepted, this feature MAY collect and report redacted telemetry but MUST block those comparisons and promotion, mark affected records with an explicit measurement-gap reason, and MUST NOT silently override current policy.
