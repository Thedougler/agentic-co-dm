...
**Constraints**: Co-DM still prep/wrapup only. Wiki **facts** stay DM-gated. Layout, token cost, helpers, the error ledger, and structure-only template rewrites are agent-owned. Do not wrap an existing command. Do not reorganize one-off files for tidiness. Do not replace the campaign `type` enum. Do not add a layout kind that duplicates an existing `type`. Do not treat System or Source Material as wiki canon. Do not add wiki templates for System or Source Material. 012/016 still bind when they apply.

**Scale/Scope**: One campaign, one table, D&D 5e. Standing rules + wrapup reflection step + error-ledger helper + sitting record + layout-kind grouping + four wiki templates. No new app, database, skill, or `type` value.
...
| IV. Tests specify behavior | Pass — fixtures observe ledger, sitting records, layout findability, reflection-as-Work, template files and pinned `type`. Not instruction wording. |
...
| VII. Do not suffocate agents | Pass — outcomes and named failures only; no prescribed reflection voice or folder taxonomy. Layout kinds name the lookup groups; they do not mandate a folder tree. Templates name jobs, not heading order. |
...
| X. Agents act autonomously by default | Pass — agents own token cost, helpers, ledger, layout, and structure-only template rewrites without waiting. |
| XI. Designated writer bounded concurrency | Pass — `session-wrapup` reflection step is design-impact; implement dispatches one writer. `AGENTS.md` / `work.md` / `wiki/templates/` / `wiki/AGENTS.md` layout rows are session-agent. This plan does not write the wrapup skill. |
...
- Provide fixture scenarios for aim-ask, gap-no-stall, reflection gate, token/helper, ledger fill/drain, mixed-dump layout (wiki kinds + System/Source Material), layout is not canon, four templates with pinned `type`, existing-page rewrite keeps facts.
