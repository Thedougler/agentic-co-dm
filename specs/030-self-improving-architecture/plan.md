...
tools/creative_lint/vale_vocab.py           # documented exclusion set {"DM"}: generated vocab never accepts a Deprecated token word (Analyze B1; research R2)
styles/config/vocabularies/CoDM/accept.txt  # regenerated without `DM` (FR-028 bookkeeping, committed with the vale_vocab.py fix)
...
tests/test_error_ledger_repairs.py, test_creative_lint_cli.py, test_wiki_cli.py, test_wiki_ops.py, test_creative_lint.py (incl. vocab-exclusion regression), test_policy_conflicts.py, test_luna_eval.py (new)
...
- **Vale (FR-022/023)**: `BasedOnStyles = CoDM, Deprecated, …` becomes `BasedOnStyles = Deprecated, …` in all 6 sections. That includes region, creature, and npc, where only `Deprecated` remains. `styles/Deprecated/*.yml` and `Vocab = CoDM` do not change. **Vocabulary fix (Analyze B1)**: Vale exempts accepted vocabulary words from every rule. The generated `accept.txt` contains `DM`, split out of the title and alias "DM Voice Notes" (`wiki/synthesis/dm-voice-notes.md`) by `_owner_terms`, so `Deprecated.DMThesis` can never fire. `tools/creative_lint/vale_vocab.py` gains one documented constant, `_RULE_TOKEN_WORDS = {"DM"}` ("words that repo-local `styles/Deprecated` tokens match; accepting them silences those rules"), and `proper_nouns` drops those terms. The fix is deterministic with one correct output. It changes the generated word list, not the `Vocab = CoDM` config or the styles, so FR-022 holds (Nick, 2026-09-24: spec Q1 covers the config and styles only). Excluding `DM` adds 0 findings across the 838 live pages (research R2). A regression in `test_creative_lint.py` renders the vocab from the live vault and asserts `DM` is absent, and runs Vale (skipped without the binary) on a scratch file with "DM Thesis" and "faction clock" using the generated vocab, asserting both `Deprecated.DMThesis` and `Deprecated.FactionClock` fire. That catches any future vocab term that silences a Deprecated rule. No `VALE_Deprecated` test changes. Drain e-212, e-214, e-215, e-216, e-218 in the same commit. In `vale_adapter.py`, repo-local (`is_custom`) findings such as `Deprecated.DMThesis` change from `deterministic_repair` + `delete_section` action to `human_repair` with no `repair_action`, so `wiki lint fix` never applies them. `wiki-lint` governs the removal or relocation (FR-023). Test in `test_creative_lint.py`.
...
- **Wiki canon (FR-047, SC-015; supersedes FR-046 rule 5's promotion bullet and SC-014 f)**: d15e99d2's consolidate `drafts` report and §3 rewrite are withdrawn. `consolidate_main` is unchanged apart from vault discovery, and `consolidate.md` §3 is deleted, not converted. Evidence and counts are in research R9. Order inside the track:
  ...
  5. **Readers**: the seven other "canon proposal" lines (`place-design/SKILL.md:55`, `writing-for-humans/SKILL.md:145`, `wiki-ingest/references/ingest-prompts.md:10`, `wiki-ingest/evals/evals.json:132`, `resolution-beats/SKILL.md:103`, `session-beats/references/agency.md:5`, `dungeon-design/SKILL.md:176`) are rewritten to "canon under the rule in `llm-wiki`" (Clarify round 4). The `superseded_by` field goes with the section (`llm-wiki` 244, 247, 310; `checks.md:93`; `wiki-capture/references/raw-format.md:20`; 0 pages use it). `tier` stays. The Foundry gates (`foundry-token` 16, `foundry-battlemap` 12, and `foundry-stage` 20, which the spec does not list) require that the source page is canon under the rule, citing `llm-wiki`, and their guardrail evals change to match. `wiki-query` trust annotations (244–261) and the `wiki-digest` Drafts list (47, 81, 143) are dropped. The other skill text and evals are listed in research R9.
...
| `CANON001` (`symbolic.py` 156–161, `rules/registry.yml` 2–14, `tests/fixtures/creative_lint/CANON001/`, `symbolic/dead_reference.md`) | Gates on `lifecycle in {rejected, dead}`; code may not gate on canon | none; conflicts are the agent's call under `llm-wiki` |
| `CANON002` age rule (`symbolic.py` 162–173 and `import datetime as dt` at line 3, `rules/registry.yml` 15–27, `tests/fixtures/creative_lint/CANON002/`, `symbolic/stale_reference.md`); `canon` category (`constants.py:7`, `rules/bundles.yml` 4, 10, 14, 24) | Flags prep links to pages with `updated` older than 30 days. That is an age rule that is neither a conflict nor a single-source fact (SC-015 c), and Clarify round 4 deleted it. With both rules gone, the category is empty | none |
| `superseded_by` (`llm-wiki` 244, 247, 310; `wiki-lint/checks.md:93`; `wiki-capture/references/raw-format.md:20`) | Only defined for `lifecycle: archived`; 0 pages use it (Clarify round 4) | none; `redirects_to` stays |
...
Net result: 2 files added, 10 deleted, and 3 ad hoc discovery paths merged into 1. FR-047 adds nothing and deletes 10 test fixtures (research R9).
...
## Spec gaps for Clarify (round 4, from 5ce37294 and v7.0.0; resolved by Clarify)

1. **"Canon proposal" wording outside the spec list**: resolved. All seven lines (`place-design/SKILL.md:55`, `writing-for-humans/SKILL.md:145`, `wiki-ingest/references/ingest-prompts.md:10`, `wiki-ingest/evals/evals.json:132`, `resolution-beats/SKILL.md:103`, `session-beats/references/agency.md:5`, `dungeon-design/SKILL.md:176`) are rewritten to "canon under the rule in `llm-wiki`" (Phase 1 "Wiki canon" step 5).
2. **CANON002 "Stale entity state"**: resolved. It is deleted with CANON001, along with its registry entry, evaluator code, test, fixtures, doc mentions, and the then-empty `canon` category (research R9). `tests/test_creative_lint_cli.py` 26/28 switches from `rule CANON001` to `rule WIKI001`, which survives and is in bundles, so "Bundles:" still prints.
3. **`superseded_by` and `tier`**: resolved. `superseded_by` is dropped, and `tier` stays (plan default confirmed).
4. **Spec citation fixes**: resolved by the spec correction (492fa2f3 and the follow-up).
