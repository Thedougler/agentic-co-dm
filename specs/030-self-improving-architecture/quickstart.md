...
| V-04b | Deprecated is a judgment repair (FR-023) | `scripts/wiki lint <scratch page containing "DM Thesis">`, then `scripts/wiki lint fix <same> --dry-run` | Finding `VALE_Deprecated.DMThesis` has `repair_class: human_repair` and no `repair_action`. `lint fix` plans no change for it. |
...
| V-04e | Consolidate reports, agent promotes (FR-046 rule 5, SC-014 f) | `scripts/wiki-lint --consolidate <scratch vault with one lifecycle: draft page> --json`; `rg -n 'auto-promot|no human judgment|base_confidence > 0.7' .agents/skills/wiki-lint/consolidate.md` | `plan.drafts` has one row with `page`, `created`, `age_days`, `base_confidence`. `plan.actions` is `[]`, and with `--approve` the page is unchanged. `rg` finds nothing. |
...
| V-08 | Health, few pages changed (SC-007) | With the lint cache and identity index current, edit 1–3 pages, then run `scripts/wiki health` and read `timing.duration_ms` | < 10 s. Warm reference 2026-09-24 (spec): 2.6 s. |
...
| V-08c | Health from scratch (SC-007) | Delete `wiki/_meta/identity-index.json` and `wiki/_meta/lint-cache.json` on a scratch branch, then run `scripts/wiki health` and read `timing.duration_ms` | Finishes. Wall time is recorded next to the SC-006 baseline (no bound). |
