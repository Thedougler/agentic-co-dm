...
- **FR-046**: Every requirement in this feature MUST be delivered under these rules. Plan, Tasks, Converge, and Review check them.
  ...
  5. **Skills first; code is diagnostic, not a prosthetic for reasoning.** "No code should be making judgement calls; that's purely the agent's domain, and those judgement calls must be backed by skills." (Nick, 2026-09-24)
     ...
     - In scope by Nick's decision: `wiki-lint --consolidate` lifecycle promotion. Today `.agents/skills/wiki-lint/consolidate.md` ("Actions" §3, "Lifecycle corrections") promotes `draft` → `reviewed` automatically when `created` > 30 days and `base_confidence > 0.7`, and labels this "no human judgment required". `scripts/wiki-lint` `consolidate_main` currently emits no actions. After this feature, age and `base_confidence` are only reported as facts. The agent decides each promotion under the `wiki-lint` skill and records its own `lifecycle_reason`. Neither `consolidate_main` nor the consolidate instructions may promote a page by threshold.
...
- **SC-014**: At Converge and at Review, the feature diff passes six checks. (a) Every new file, module, command, subcommand, config key, or data store maps to a named existing module that it replaces, extends, or consolidates, with the reason recorded in plan.md. (b) No two modules own the same concern. (c) Every duplicate, dead, or superseded path that plan.md identifies is deleted and no longer referenced. (d) Every new or changed piece of code is diagnostic (it detects, measures, checks, indexes, or reports) or applies only fully deterministic fixes, and plan.md justifies it as such; no code decides on the agent's behalf. (e) Every agent judgment this feature introduces or relies on (duplicate choice, same-root-cause beyond a mechanical match, promotion, repair choice) names the skill that governs it. (f) `wiki-lint --consolidate` promotes no page on its own: its output reports each `draft` page's age and `base_confidence`, it contains no promotion action, and `consolidate.md` contains no threshold-based auto-promotion rule. No numeric line limit applies.
