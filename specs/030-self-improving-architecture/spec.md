...
- **FR-007**: Recording a failure whose `source` equals an open entry's `source` and whose normalized cause (casefolded, whitespace collapsed, digits and quoted or backticked literals removed) equals that entry's normalized cause MUST append one occurrence to that entry's `evidence` rather than create a new id. An explicit attach to a named open entry MAY override the cause match but MUST be refused when the `source` differs.
...
- **FR-024**: Scoped wiki lint and health MUST resolve identity by: cached vault index → selected pages → candidates (pages sharing a cheap exact signal — slug, title, aliases, manifest identity — or passing the cached character-profile prefilter, similarity > 0.6) → expensive content comparison only for those candidates.
...
- **SC-014**: At Converge and at Review, the feature diff passes five checks. (a) Every new file, module, command, subcommand, config key, or data store maps to a named existing module that it replaces, extends, or consolidates, with the reason recorded in plan.md. (b) No two modules own the same concern. (c) Every duplicate, dead, or superseded path that plan.md identifies is deleted and no longer referenced. (d) Every new or changed piece of code is diagnostic (it detects, measures, checks, indexes, or reports) or applies only fully deterministic fixes, and plan.md justifies it as such; no code decides on the agent's behalf. (e) Every agent judgment this feature introduces or relies on (duplicate choice, same-root-cause beyond a mechanical match, promotion, repair choice) names the skill that governs it. No numeric line limit applies.
