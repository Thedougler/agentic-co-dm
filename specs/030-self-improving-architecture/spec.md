...
3. **Given** the simplified Vale configuration, **When** scoped wiki lint runs on any page, **Then** Vale loads without a missing-style error and applies only ai-tells, write-good, proselint, and Readability.
...
- The Vale `Deprecated` style currently catches retired terms (`DM Thesis`, faction/agenda clock); when it is removed, the Python retired-term check (FR-023) must report the same pages. A page with `DM Thesis` still gets a deterministic delete-section repair, and faction/agenda clock findings keep their existing Python rule ids.
...
- **FR-022**: Vale configuration MUST base prose checks only on ai-tells, write-good, proselint, and Readability, and MUST NOT reference the `CoDM` style or the repo-local `Deprecated` style. `Vocab = CoDM` (the generated proper-noun vocabulary, not a style) stays.
- **FR-023**: Repository-specific structural and semantic checks (D&D/wiki structure, ownership, identity, links, templates, canon, retired terms) MUST live in the Python wiki tooling (`tools/wiki_ops/`, `scripts/wiki-lint`), not in custom Vale styles. `styles/Deprecated/` MUST be deleted. Its `DM Thesis` check MUST move into the existing `scripts/wiki-lint` retired-guidance check, keeping a deterministic delete-section repair. The faction/agenda-clock check already exists there and MUST NOT be duplicated. `styles/config/vocabularies/CoDM/accept.txt` stays a Vale vocabulary generated by `tools/creative_lint/vale_vocab.py`.
