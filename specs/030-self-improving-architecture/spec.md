...
- **FR-047**: The wiki page lifecycle, promotion, and confidence machinery MUST be replaced by Nick's canon rule, in precedence order:
  ...
  4. **Delete the machinery.**
     - Fields: `lifecycle`, `lifecycle_changed`, `lifecycle_reason`, `base_confidence`, the ad hoc `canon_status`, and `superseded_by` (meaningful only for the removed `archived` state; 0 pages carry it). `tier` is unchanged. These are removed from the 27 templates in `wiki/templates/*.md` and from the `required:` lists in the 15 `wiki/templates/contracts/*.yml` that name `lifecycle`. They are also removed from every page in one mechanical, deterministic migration: 833 live pages plus 15 archived/meta pages. `reveal`, `visibility`, `truth`, `invention`, and entity `status` stay; they are not canon-promotion fields.
     - Skill text: `llm-wiki/SKILL.md` "Confidence and Lifecycle" (lines 231–313) and the `OBSIDIAN_ALLOWED_LIFECYCLES` / `OBSIDIAN_REQUIRED_TRUST_FIELDS` keys; `wiki-lint/checks.md` §12 (lines 75–98); `wiki-lint/consolidate.md` §3 (lines 44–48), the `lifecycle` line in step 3 (line 13), the `lifecycle: draft` report frontmatter (lines 75–76), and `lifecycle_updates` in the log line (line 92); the frontmatter/formula blocks in `wiki-ingest/SKILL.md` (lines 503–517) and `wiki-ingest/references/url-sources.md` (lines 148–196); the same blocks in `wiki-update`, `wiki-capture` (and `references/raw-format.md`), `wiki-research`, `wiki-synthesize`, `wiki-dedup`, `wiki-import`, `wiki-digest`, `obsidian-markdown`, `vault-skill-factory`, the seven `*-history-ingest` skills, `lore-design` lines 93 and 105, and the `lifecycle` mentions in the design skills; `templates/work.md` "`lifecycle` stays `proposed` until that decision"; `superseded_by` in `llm-wiki/SKILL.md` (lines 244, 247, 310), `wiki-lint/checks.md` §12d (line 93), and `wiki-capture/references/raw-format.md` (line 20). About 288 skill lines across 39 files.
     - Canon-proposal wording: seven maintained lines that still call unresolved content a "canon proposal" are rewritten to "canon under the rule in `llm-wiki`": `place-design/SKILL.md` line 55, `writing-for-humans/SKILL.md` line 145, `wiki-ingest/references/ingest-prompts.md` line 10, `wiki-ingest/evals/evals.json` line 132, `resolution-beats/SKILL.md` line 103, `session-beats/references/agency.md` line 5, and `dungeon-design/SKILL.md` line 176.
     - Code: `tools/lint_wiki.py` `OWNER_LIFECYCLES`/`DEFAULT_LIFECYCLES` (lines 24–25), `lifecycle` in `CAMPAIGN_REQUIRED` (line 30), the `bad_lifecycle` key (line 41), lifecycle parsing from `wiki/AGENTS.md` (lines 198–208), `--allow-lifecycle` and `--required-trust-field` (lines 320–322), the allowed-lifecycle and trust-field setup (lines 567–578), `bad_lifecycle` (lines 615–618), `missing_trust` (lines 649–658, 852), and the `lifecycle` report column (line 817); `scripts/wiki-lint` `--allow-lifecycle` pass-through (lines 111, 129–130); `tools/wiki_ops/worklist.py` `bad_lifecycle` (line 14); `tools/creative_lint/evaluators/symbolic.py` lifecycle checks (lines 89, 117) `CANON001` (lines 156–161) with its `rules/registry.yml` entry (lines 2–14), and `CANON002` (lines 162–173: an age rule that flags prep links to pages not updated in 30+ days; it is not part of the three-rule canon and SC-015 (c) forbids it) with its `rules/registry.yml` entry (lines 15–27); the lifecycle half of `WIKI002` in `rules/registry.yml` (lines 48–49); `tools/creative_lint/engine.py` lifecycle exemptions (lines 145–150; no rule declares one); `tools/wiki_ops/identity.py` `lifecycle` (lines 108, 139, 202); `tools/check_wiki_pages.py` lifecycle check (lines 25, 61–66); `scripts/wiki-reveal` line 54; and the `lifecycle`/`base_confidence` defaults in `scripts/wiki-bulk-ops` (lines 971–974) and `scripts/ingest-raw.py` (line 74). About 75 code lines across 12 files.
     - Tests and evals that exist only for the machinery are deleted: fixtures `tests/fixtures/creative_lint/symbolic/invalid_lifecycle.md`, `tests/fixtures/creative_lint/WIKI002/ambiguous_proposed_state.md`, and the `tests/fixtures/creative_lint/CANON002/` directory (3 fixtures), `CANON002` in the rule set asserted at `tests/test_creative_lint.py` line 47 and in `docs/creative-linting.md` line 125, and the lifecycle/`base_confidence` assertions in `tests/test_wiki_ops.py`, `test_creative_lint.py`, `test_creative_lint_cli.py`, `test_wiki_bulk_ops.py`, `test_wiki_cli.py`, and `test_error_ledger_repairs.py`. Other assertions are kept and made independent of the removed fields.
  5. **Rewrite or drop every reader.**
     - Foundry gates: `foundry-token/SKILL.md` line 16, `foundry-battlemap/SKILL.md` line 12, and `foundry-stage/SKILL.md` line 20 (same gate, `accepted` or `canon`) require `lifecycle: accepted`, and today only 7 pages pass. They are rewritten to require that the source page is canon under the rule (agent judgment citing `llm-wiki`). Their evals `foundry-token/evals/evals.json` (lines 28–31) `foundry-battlemap/evals/evals.json` (lines 18–21), and `foundry-stage/evals/evals.json` (line 21) are rewritten to match.
     ...
     - Evals that assert lifecycle fields are updated: `wiki-import` line 10, `wiki-ingest` line 85, `vehicle-design` lines 22 and 88, `city-design` line 76, `spell-design` lines 22 and 84, `narrative-islands` line 136, and `wiki-dashboard` line 6. With the Foundry evals, that is 10 eval files.
...
- **SC-015**: At Converge and at Review, (a) maintained surfaces contain no page lifecycle, promotion, or confidence fields or rules. The maintained surfaces are `wiki/` outside `_raw/`, `wiki/templates/`, `.agents/skills/`, `AGENTS.md`, `wiki/AGENTS.md`, `docs/agents/`, `tools/`, `scripts/`, `rules/`, `tests/`, and skill `evals/`. A search for `lifecycle:`, `lifecycle_changed`, `lifecycle_reason`, `base_confidence`, `canon_status`, and `superseded_by` in page or template frontmatter, and for their rules in skills and code, returns 0 hits; ACTIVE/SHADOW rule status in `creative_lint` is unrelated and exempt. (b) The FR-047 rule text appears verbatim in exactly one maintained file, `.agents/skills/llm-wiki/SKILL.md`, and the AGENTS.md files point to it. (c) Every canon-related lint finding is either a reported conflict or a single-source fact, and no code path assigns or gates on canon.
