...
- Q: What decides wiki canon, and what happens to page lifecycle, promotion, and confidence? → A: Decided by Nick, 2026-09-24: canon is three rules in precedence order. (1) Something is canon if the DM says so. (2) Otherwise, it's canon if it's present in the wiki in multiple places. (3) Otherwise, it's canon if it doesn't conflict with the wiki. That is the entire system. The lifecycle states, `base_confidence`, `lifecycle_changed`, `lifecycle_reason`, age rules, and consolidate promotion are the kind of overengineering this feature removes. Recorded in FR-047 and SC-015; this supersedes FR-046 rule 5's consolidate-promotion bullet and SC-014 (f).
...
- **Canon impact**: The rule that decides canon changes to FR-047's three rules, and page lifecycle and confidence metadata is deleted. No campaign fact is added, removed, or edited. Constitution X's sentence "Everything else it writes is a canon proposal … canon only once the DM accepts it" conflicts with rules 2 and 3. This feature does not edit the constitution (out of scope), so the plan MUST record the conflict as a constitution amendment for Nick through `speckit-constitution`.
...
- **FR-047**: The wiki page lifecycle, promotion, and confidence machinery MUST be replaced by Nick's canon rule, in precedence order:
  ...
  > 1. Something is canon if the DM says so. 2. Otherwise, it's canon if it's present in the wiki in multiple places. 3. Otherwise, it's canon if it doesn't conflict with the wiki.

  That is the entire system.
  1. **One owner, verbatim.** The rule text appears verbatim only in `.agents/skills/llm-wiki/SKILL.md`, which already owns "confidence/lifecycle" (its description, line 5). It replaces that file's "Confidence and Lifecycle" section (lines 231–313). Other surfaces link to it and never restate it: root `AGENTS.md` line 87 ("Canon owner: constitution principle X …"), `wiki/AGENTS.md` lines 54 and 64, and `docs/agents/work.md` line 7.
