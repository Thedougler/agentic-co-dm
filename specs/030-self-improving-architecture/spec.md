...
| Scoped identity | `tools/wiki_ops/identity.py` walks the whole vault (`vault.rglob("*.md")`) and pairwise similarity; open entries e-206 (45.9 s whole-vault, 44.3 s identity), e-208 (52 s, health timeout), e-213 (~38 s to lint 2 pages) |
| Vale | `.vale.ini` lists `Packages = ai-tells, proselint, write-good, Readability` but `BasedOnStyles` also names `CoDM` (directory absent from `styles/`) and the repo-local `Deprecated` style (`styles/Deprecated/DMThesis.yml`, `FactionClock.yml`); `Vocab = CoDM` (`styles/config/vocabularies/CoDM/accept.txt`). Open entries e-212, e-214, e-215, e-216, e-218 |
...
3. **Given** a defect that cannot be fixed in the current task, **When** the agent records it, **Then** `errors.md` gets one entry with `id`, `cause`, `source`, `evidence`, or the evidence is appended to an existing entry with the same root cause.
...
- The Vale `Deprecated` style currently catches retired terms (`DMThesis`, `FactionClock`); removing it without a replacement would drop those checks.
...
- **FR-006**: `errors.md` MUST contain only reusable, unresolved defects, each with exactly the fields `id`, `cause`, `source` (authoritative file or component), and `evidence` (one or more occurrences).
- **FR-007**: Recording a failure whose `source` and cause match an open entry MUST append evidence to that entry rather than create a new id.
- **FR-008**: A verified fix MUST drain its entry in the same change that lands the fix.
- **FR-009**: `scripts/error-ledger.py` MUST report the recurrence metric: the number of recorded occurrences whose root cause was already open.
- **FR-010**: On first run of the new format, existing open entries MUST be migrated to the four fields and duplicates merged (OMP: e-205, e-207, e-209, e-211; Vale `CoDM`: e-212, e-214, e-215, e-216, e-218; identity performance: e-206, e-208, e-213); already-drained entries MUST be removed.
...
- **FR-013**: All eval records MUST use one schema: `id`, `prompt`, `skill`, optional `files`/`context`/`outputs` inputs, `expected_output`, and `assertions[]` of `{type, text}`; the 143 `expectations[]` records MUST be converted.
...
- **FR-022**: Vale configuration MUST base prose checks only on ai-tells, write-good, proselint, and Readability, and MUST NOT reference the `CoDM` style.
- **FR-023**: Repository-specific structural and semantic checks (D&D/wiki structure, ownership, identity, links, templates, canon, retired terms) MUST live in the Python wiki tooling (`tools/wiki_ops/`, `scripts/wiki-lint`), not in custom Vale styles. [NEEDS CLARIFICATION: Should the repo-local `Deprecated` Vale style (`styles/Deprecated/DMThesis.yml`, `FactionClock.yml`) and the `CoDM` vocabulary (`styles/config/vocabularies/CoDM/accept.txt`) also be removed from Vale — migrating retired-term checks into Python tooling — or kept, with only the missing `CoDM` style reference removed?]
...
- **Eval record**: One case in a skill's `evals/evals.json`: id, prompt, skill, inputs, expected output, typed assertions.
...
- **Error entry**: `errors.md` line with `id`, `cause`, `source`, `evidence[]`; drained when its fix is verified.
...
- **SC-007**: Scoped lint of one page on a warm, unchanged vault finishes in under 5 seconds (currently ~38 s for two pages), and `wiki health` completes within its timeout.
...
- The 5-second target in SC-007 is measured on the maintainer's normal workstation against the current vault size.
...
- Constitution XIII currently requires every friction to be logged in `errors.md` and diagnosed by a different agent. [NEEDS CLARIFICATION: This spec limits `errors.md` to reusable unresolved defects and has the agent that meets friction fix it and continue. Should the constitution be amended (tracked separately) to match, or should FR-006–FR-008 be narrowed to fit current principle XIII?]
