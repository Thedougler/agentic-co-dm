...
**Input**: Design documents from `/specs/030-self-improving-architecture/` (plan.md at 887e6c47, consolidated under FR-046/SC-014 and FR-047/SC-015; spec.md at c6f53f02; constitution v7.0.0 at f489409d). Line numbers are at the branch head after 4d2b68c5 (research R9 notes the shifts); locate by symbol if they drift.
...
- [X] T012 [US1] Drain OMP entries e-205, e-207, e-209, and e-211 from `errors.md` with the current CLI, which still requires the flag at this point: `python3 scripts/error-ledger.py error drain --id <id> --cause-fixed true`. This runs before T074 removes `--cause-fixed`; never pass it after that. Do it in the same commit as T006 and T011 (FR-008).
...
- [X] T014 [US1] Drain Vale entries e-212, e-214, e-215, e-216, and e-218 from `errors.md` with the current CLI (`error drain --id <id> --cause-fixed true`, before T074), in the same commit as T007 and T013 (FR-008)
...
- [ ] T034 [P] [US2] Add a case to `tests/test_policy_conflicts.py` (SC-008, V-11) that runs `scripts/check-current-commands --json` over `specs/026-agent-autonomy-scope/` and fails on any cited `scripts/…` or `tests/…` path that does not exist, and separately fails on any `AGENT00[1-3]` in those docs. Confirm it fails before T037 and T036.
- [ ] T035 [US2] Create `tests/test_luna_eval.py`, the one new test file (plan "New files"). Cover:
  ...
  Confirm the schema, metrics, and grading cases fail before T039–T041.
...
- [ ] T037 [P] [US2] Extend `scripts/check-current-commands` (research R8): also check cited `scripts/…` and `tests/…` paths, and add `specs/026-agent-autonomy-scope/` to its scan roots. Make T034 pass.
- [ ] T038 [US2] Replace the `git rev-parse --show-toplevel` root in `scripts/luna-eval` (line ~47) with `tools/wiki_ops/cli.py` `repo_root`, started from the script's own location, so it runs from any working directory. Keep the existing flags (FR-027, contracts/eval-run.md). T081 later makes the package location the helper's default.
- [ ] T039 [US2] Add the evals.json schema check to `load_eval` in `scripts/luna-eval` (plan "Evaluator"; chosen over `quick-validate.py`): validate the whole file against data-model §2 and exit 2 naming each bad record before any subject run, with the error shape in contracts/eval-run.md "Schema check".
- [ ] T040 [US2] Make `scripts/luna-eval` write `metrics.json` into each `run-<n>/` directory, derived from `events.jsonl` with the names confirmed in T005. The keys are exactly those in data-model §3: `tool_calls` (by kind), `total_tool_calls`, `retries` (a failed command re-run with identical argv), `invocation_errors` and `invocation_error_commands` (rule as in T035), `duplicate_actions`, `tokens` {`input`,`output`,`total`}, `completion_reason` ∈ `ok|no_output|usage_limit|error`, `model` and `effort` (the values `luna-eval` actually passed to codex), and `skills_read`. An event that is absent is recorded as `null`, never guessed (research R5).
- [ ] T041 [US2] Make `scripts/luna-eval` grade `skill_selected` assertions into `grading.json`. An item passes when the first owner `SKILL.md` in `metrics.json` `skills_read` matches `text`. Write each item as `{text, passed, evidence, type}` (data-model §3). Make T035 pass.
- [ ] T042 [US2] In `scripts/luna-eval`, keep exit codes 0/3/4/5 unchanged. On exit 3 or 5, keep completed runs. On a rerun, skip any `run-<n>/` that already has `timing.json` with exit 0 (contracts/eval-run.md, spec Edge Case).
...
- [ ] T045 [US2] Update `.agents/skills/skill-creator/SKILL.md` so that "Running and evaluating test cases" calls `scripts/luna-eval` once per eval and config. Delete the "Description optimization" section and the `run-loop.py` / `run-eval.py` line (~460) (FR-012, FR-015, research R5).
- [ ] T046 [US2] Delete `.agents/skills/skill-creator/scripts/run-eval.py`, `run-loop.py`, `improve-description.py`, and `generate-report.py` (it only renders `run-loop.py` output). Keep `utils.py` (`package-skill.py` uses it). Then `rg --hidden "run-eval|run-loop|improve-description|generate-report" .agents/ AGENTS.md docs/ scripts/ tools/ tests/` and remove the remaining references, including `.agents/skills/vault-skill-factory/SKILL.md` line ~106.
- [ ] T047 [US2] Run a one-off conversion over all `.agents/skills/*/evals/evals.json`. The script is not committed (plan: VIII does not apply to one-offs). Each `expectations[]` string `s` becomes `{"type":"behavior","text":s}`, and `expectations` is removed. Rename `qualitative`→`quality` and `structural`→`structure`; keep `scope`, `handoff`, and `coverage` (FR-014). Preserve `core`, `subject_skill`, `trajectory_records`, and `principles`. `expected_output` stays optional, with no text invented (FR-013).
- [ ] T048 [US2] Add `"skill_name"` (equal to the directory name) to `.agents/skills/{session-beats,wiki-capture,wiki-context-pack,wiki-ingest,wiki-lint,wiki-query,wiki-update}/evals/evals.json` (FR-013)
- [ ] T049 [US2] Edit the Work-gate, chat-proposal approval, read-side mutation, and retired-path wording by hand, record by record, in the 13 files found by `rg -il "work gate|chat proposal|approval" .agents/skills/*/evals/evals.json`. This includes `.agents/skills/place-design/evals/evals.json` eval 1. Keep each assertion and phrase its current behavior positively (e.g. "writes without asking first") (FR-017, spec Edge Cases).
- [ ] T050 [US2] Add at least one `skill_selected` eval (`{"type":"skill_selected","text":"<dir name>"}`) to each of the six FR-003 owners fixed by Clarify (cae8bb44): `.agents/skills/{place-design,faction-design,session-beats,run-guide,wiki-query,wiki-lint}/evals/evals.json`. Write each prompt from that skill's own ownership text (FR-003, FR-014, FR-015, SC-005).
- [ ] T051 [US2] Verify V-09 with `pytest tests/test_luna_eval.py` (schema over all 60 files and the Work-gate wording case pass) and V-11 with `pytest tests/test_policy_conflicts.py`.
- [ ] T052 [US2] Verify V-10. Run the `place-design` `skill_selected` record directly with `scripts/luna-eval ... --out /tmp/030/iteration-1`, then through the skill-creator workflow. Both run dirs must have the same files and keys (`timing.json`, `metrics.json`, `grading.json`), and the item must be `passed: true`. Then drain e-210 from `errors.md` with the current CLI (`error drain --id e-210 --cause-fixed true`, before T074), in the same commit as T038–T046 (research R3, FR-008).
...
- [ ] T057 [US4] Add identity-index tests to `tests/test_wiki_ops.py`, using a temp vault built in the test:
  ...
  Confirm (e) and (g) fail before T058–T060.

### Implementation for User Story 4

- [ ] T058 [US4] Add load and save for `wiki/_meta/identity-index.json` to `tools/wiki_ops/lint_cache.py`, reusing its `cache_path`, `sha256_file`, versioned load, and atomic writer (research R8; not in `identity.py`). Use data-model §4 exactly:
  - Rows carry no `lifecycle` (data-model §4 "No lifecycle"; `version` stays 1). This task runs after T027.
...
- [ ] T059 [US4] In `tools/wiki_ops/identity.py`, replace the whole-vault read in `scan_identities` with candidate selection from index rows (research R4 steps 1–5; `os.scandir` + `stat` walk), on top of T024 (no `lifecycle` field). Candidates share the same `type`, have no redirect, and meet one of: a norm title or alias match, a stem match, shared manifest provenance with stem ratio > 0.7, a merge transition, or the cached profile prefilter `2·|A∩B| / (lenA+lenB) > 0.6` (FR-024 as clarified). Make `resolve_identity` (mutation gate) and `_path_for` read the index too. Thresholds only list candidates and set `status: ambiguous`, which gates mutations; resolution goes through `.agents/skills/wiki-dedup/SKILL.md` and `.agents/skills/wiki-lint/SKILL.md` (plan "Judgments").
- [ ] T060 [US4] Add the pair-ratio cache to `tools/wiki_ops/identity.py`: read `pairs` through `lint_cache.py`, run `SequenceMatcher` only on candidate pairs that are uncached, and read only those candidate bodies (research R4 step 6, R7). Make T057 pass.
- [ ] T061 [US4] Add `"identity": {"status","ambiguous","scanned","compared","index":{"hits","misses"}}` to `scripts/wiki-lint --json` output, where `scanned` = selected and `compared` = |selected ∪ candidates| (data-model §4, SC-007)
- [ ] T062 [US4] Delete `scripts/wiki-identity` (research R8). Move `tests/test_wiki_ops.py` line 109 (drop it from the `--help` contract loop), line 115 (`scan --json` ambiguous case → call `scan_identities` or read the `wiki lint` `identity` block), and line 514 (`resolve` case → call `resolve_identity`) onto the library or `wiki lint`.
- [ ] T063 [US4] Keep `wiki/_meta/lint-cache.json`, `wiki/_meta/identity-index.json`, and `styles/config/vocabularies/CoDM/accept.txt` tracked as FR-028 bookkeeping (data-model §4 "Tracking"): no `.gitignore` change, and each is committed with the change that caused it. Commit the first generated `identity-index.json` with T058–T061.
- [ ] T064 [US4] Verify V-05 (read run 1 `timing.duration_ms` < 5000, `identity.compared` far below the page count; run 2 `cache.hits: 1`), V-06 (diff against `/tmp/030/identity-before.json` and `/tmp/030/lint-before.json` from T002, allowing only the findings added by T016 and T025, those removed by T018 and T024, and the dropped `lifecycle` field), and V-07 (corrupt index rebuilt). Record the V-05 numbers in the `specs/030-self-improving-architecture/quickstart.md` V-05 row.
- [ ] T065 [US4] Run V-08: with the lint cache and identity index current, edit 1–3 pages on a scratch branch, run `scripts/wiki health`, and read `timing.duration_ms` (< 10 000). Then run V-08c: delete both `wiki/_meta/identity-index.json` and `wiki/_meta/lint-cache.json` on the scratch branch, run `scripts/wiki health`, confirm it finishes with no harness timeout, and record its `timing.duration_ms` next to the SC-006 baseline in `specs/029-agent-loop-closure/quickstart.md` (after T055; no bound).
- [ ] T066 [US4] Run V-08b: on a scratch branch with the identity index current, change the rules digest (e.g. add a comment line to `.vale.ini`), run `scripts/wiki health`, and read `timing.duration_ms` (< 50 000), `lint.cache.misses` (≈ page count), and `identity.index.misses` (== 0). Record the result in the `specs/030-self-improving-architecture/quickstart.md` V-08b row (pre-change reference 82.3 s). Discard the scratch branch.
- [ ] T067 [US4] Drain identity entries e-206, e-208, and e-213 from `errors.md` with the current CLI (`error drain --id <id> --cause-fixed true`, before T074), in the same commit as T057–T063 (FR-008)
...
- [ ] T068 [P] [US5] Add one friction-rule paragraph to `AGENTS.md` next to "Capability loop" (line ~72): act → friction → identify cause → fix the authoritative source (smallest change) → verify → continue the original task. Add no new workflow, command, or skill (FR-001, FR-002, FR-005).
- [ ] T069 [P] [US5] Add the friction branch (act → friction → fix source → re-observe → continue) to the capability loop in `docs/agents/hybrid-sdd.md`, linking to the `AGENTS.md` paragraph rather than restating it (FR-001, constitution XVI)
- [ ] T070 [US5] Run V-17. On a scratch branch, make one owner `SKILL.md` name a retired command path, then run 5 cold `scripts/luna-eval` subjects on one of that owner's evals. At least 4/5 must fix the source, add a regression, and finish the owner task with no new `errors.md` entry. Record the result in the `specs/030-self-improving-architecture/quickstart.md` V-17 row. Discard the scratch branch. This runs after T079 so the new ledger format is in place.
...
- [ ] T071 [US6] Add ledger tests to `tests/test_error_ledger_repairs.py`. Each one drives `scripts/error-ledger.py` via subprocess on a scratch `errors.md` and covers:
  ...
  Plan default, error-ledger CHK011: an "identical occurrence" means the same `sitting` and `detail`. Confirm the tests fail before T072.

### Implementation for User Story 6

- [ ] T072 [US6] Rework the entry model in `scripts/error-ledger.py` (data-model §1). Keep the `# Error ledger` heading, a blank line, then one JSON object per line with sorted keys. The fields are exactly `id` (`"e-<n>"`, unique, new ids from `next_error_id`), `cause` ("Never rewritten when an occurrence is added"), `source` (repo-relative path or `external:<name>`), and `evidence` ("list of `{sitting, detail}`, length ≥ 1"). There is no `status` or `cause_fixed`. The only automatic match is the same `source` with `cause.strip()` equal; no normalized key or other heuristic (FR-007, research R3).
...
- [ ] T080 [US7] Add one subprocess test per rule class to `tests/test_wiki_cli.py` (FR-039):
...
- [ ] T090 [US8] Change `.agents/skills/skill-creator/scripts/aggregate-benchmark.py` to report only, per eval, task-outcome regressions and assertions that passed for the incumbent and fail for the candidate, plus the medians of `total_tool_calls`, `retries`, `tokens.total`, and latency (from `timing.json`). It prints no verdict and no mean ± stddev, and refuses a comparison whose runs differ in `model` or `effort` (data-model §3, FR-044, contracts/eval-run.md "Promotion report").
- [ ] T091 [US8] Add the promotion rule to `.agents/skills/skill-creator/SKILL.md` (FR-044, SC-012): one run per config on the same eval ids, with the same `model` and `effort` as the FR-019 baseline; refuse on any worse per-eval task outcome or any incumbent-passing assertion that now fails; promote only on a lower median of tool calls, retries, or tokens with none of the three higher; do not use mean ± stddev. Keep the skill no longer than before (SC-013) by replacing the text it supersedes.
- [ ] T092 [US8] Pick one of the six owners. Draft one small candidate edit to `.agents/skills/<owner>/SKILL.md` in the working tree, drawing on its US3 baseline run dirs under `/tmp/030/baseline/`: the current instruction, failed and successful tasks, tool results, assertion failures, user corrections, and token/tool-call cost (FR-043)
- [ ] T093 [US8] Snapshot the incumbent skill directory from HEAD to `/tmp/030/incumbent/<owner>/` (`git archive HEAD .agents/skills/<owner> | tar -x -C /tmp/030/incumbent`). Run `scripts/luna-eval --config old_skill --subject-skill /tmp/030/incumbent/.agents/skills/<owner>` and `--config with_skill` once each on the same eval ids with the baseline `--model`/`--effort`, plus the cross-skill baseline cases from T053 (FR-042, spec Edge Case).
- [ ] T094 [US8] Run `aggregate-benchmark.py` on the two configs. Decide keep or refuse by following the rule in `skill-creator/SKILL.md` (T091); on refuse, `git checkout -- .agents/skills/<owner>/SKILL.md`. Record the decision and cite the skill in the `specs/030-self-improving-architecture/quickstart.md` V-18 row.
...
- [ ] T095 [US9] Record `wc -l .agents/skills/{place-design,faction-design,session-beats,run-guide,wiki-query,wiki-lint}/SKILL.md` to `/tmp/030/skill-lines-before.txt`, and snapshot those dirs to `/tmp/030/incumbent-us9/` for old_skill runs
- [ ] T096 [US9] Make `scripts/manifest.py` report the manifest facts named in recorded trajectories (facts only, no interpretation), with a regression in `tests/test_wiki_ops.py` (FR-029, FR-011)
- [ ] T097 [US9] Move deterministic repair bookkeeping (fixes with exactly one correct output) into `wiki lint fix` in `scripts/wiki` and `tools/wiki_ops/cli.py`, with a regression in `tests/test_wiki_cli.py` (FR-029, FR-011). Repair choices stay in the skills.
- [ ] T098 [P] [US9] Shorten `.agents/skills/place-design/SKILL.md`. Remove only deterministic text now covered by T081, T087, T096, and T097 (path discovery, health facts, manifest facts, repair bookkeeping). Keep every judgment, and make sure the five FR-040 answers are present.
- [ ] T099 [P] [US9] Shorten `.agents/skills/faction-design/SKILL.md` in the same way as T098
- [ ] T100 [P] [US9] Shorten `.agents/skills/session-beats/SKILL.md` in the same way as T098
- [ ] T101 [P] [US9] Shorten `.agents/skills/run-guide/SKILL.md` in the same way as T098
- [ ] T102 [P] [US9] Shorten `.agents/skills/wiki-query/SKILL.md` in the same way as T098
- [ ] T103 [P] [US9] Shorten `.agents/skills/wiki-lint/SKILL.md` in the same way as T098 (it builds on T078, and keeps the `Deprecated.*`, broken-image, and identity judgments)
- [ ] T104 [US9] Verify V-19. Line counts must not exceed `/tmp/030/skill-lines-before.txt`. Rerun each skill's evals with `scripts/luna-eval` `--config with_skill` against `--config old_skill --subject-skill /tmp/030/incumbent-us9/...`, report with `aggregate-benchmark.py`, and apply the `skill-creator` rule (T091). Revert any skill it refuses.
...
- **Foundational (Phase 2)**: Depends on Setup. T004 gates US5 and US6. T005 gates T040.
...
- **US1 (P1)**: After Foundational. There are no story dependencies. It is the MVP. Its canon track (T023–T032) is not gated (constitution v7.0.0 meets X) and runs after T016.
- **US2 (P1)**: After US1, because eval runs need green checks. T036 and T034/T037 can start any time.
- **US3 (P1)**: After US2, because it needs `metrics.json` and `grading.json` (T040–T043)
- **US4 (P1)**: After US1 (Vale fixed, lint rules folded, canon migration and cache refresh done, so the index never carries `lifecycle` and SC-007 timings run on migrated pages). It is independent of US2 and US3, except T065, which writes after T055.
- **US5 (P2)**: T068 and T069 are independent. T070 needs US2 (luna-eval) and T079 (new ledger format).
- **US6 (P2)**: After US1, US2, and US4, whose drains (T012, T014, T052, T067) use the current CLI and must land before T074 removes `--cause-fixed` and before migration (T076)
- **US7 (P2)**: After US4 (the `wiki-lint` output shape) and US6 (`scripts/wiki` `error list` callers). T088 needs T038–T042.
- **US8 (P3)**: After US2, US3, and US5
- **US9 (P3)**: After US7 (T081, T087), US3, and US8 (T091). T103 comes after T078.
...
- Tasks on the same file run in sequence (e.g. T038→T042 `scripts/luna-eval`; T072→T075 `scripts/error-ledger.py`; T082→T087 `scripts/wiki`; T059→T060 `tools/wiki_ops/identity.py`; T009 and T010 share `tests/test_wiki_ops.py`; T024→T028 edit `tools/lint_wiki.py` and the tests after T016).

### Parallel Opportunities

- T003 runs alongside T002.
- US1: T006 ∥ T007 ∥ T008 ∥ T009 (then T010 in the same file). The OMP track (T011–T012) is independent of the Vale track (T013–T015) and the one-linter track (T016–T020), apart from sharing `errors.md` at commit time. The canon track follows the one-linter fold: T023→T024→T025→T026→T027→T028 in order, then T029 ∥ T030 ∥ T032 (then T031); T021 can run any time in US1. All of it finishes before US4.
- US2: T034 ∥ T036 ∥ T037 ∥ T043 ∥ T044 ∥ the `scripts/luna-eval` chain T038–T042.
- US4 can run alongside US2 and US3 once US1 is done.
- US5: T068 ∥ T069. US6: T077 ∥ T078.
- US7: T088 ∥ T081–T087.
- US9: T098–T103 in parallel (one skill each).
...
```bash
Task: "T036 Rewrite 026 references in specs/026-agent-autonomy-scope/*"
Task: "T043 Update .agents/skills/skill-creator/agents/grader.md"
Task: "T044 Update .agents/skills/skill-creator/references/schemas.md"
```

## Parallel Example: User Story 9

```bash
Task: "T098 Shorten .agents/skills/place-design/SKILL.md"
Task: "T099 Shorten .agents/skills/faction-design/SKILL.md"
Task: "T100 Shorten .agents/skills/session-beats/SKILL.md"
Task: "T101 Shorten .agents/skills/run-guide/SKILL.md"
Task: "T102 Shorten .agents/skills/wiki-query/SKILL.md"
```
...
- Plan default kept: error-ledger CHK011, an identical occurrence means the same `sitting` + `detail` (T071).
...
- Round-3 Clarify gap 4 (consolidate promotion): resolved by 27039b9d, then superseded by FR-047 (5ce37294). There is no promotion: `consolidate.md` §3 is deleted (T021) and `consolidate_main` gets no `drafts` report.
- Round-4 Clarify gaps 1–4: resolved by Clarify (spec c6f53f02, plan 887e6c47): (1) the 7 extra "canon proposal" lines become "canon under the rule in `llm-wiki`" (T031); (2) `CANON002` is deleted with `CANON001` and the `canon` category (T024, T028); (3) `superseded_by` is dropped (T027, T029, T032) and `tier` is unchanged; (4) spec citation fixes are applied. Nothing is pending except error-ledger CHK011 as a plan default.
