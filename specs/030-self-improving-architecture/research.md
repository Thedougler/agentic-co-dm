...
- **Decision**: store one JSON line per entry, `{"id","cause","source","evidence":[{"sitting","detail"}]}`, keeping the `# Error ledger` heading. `append --source S --cause C --sitting X [--detail D]` attaches to an open entry when `source` is equal and `cause_key(C) == cause_key(entry.cause)`. `cause_key` casefolds, collapses whitespace, and drops digits and backtick- or quote-wrapped literals. `--attach e-N` forces the target. `drain --id e-N` deletes the entry. The `--cause-fixed true` flag is kept as an accepted no-op so existing documented invocations do not break; it is removed from the docs.
...
- **Decision**: `scripts/luna-eval` stays the only runner. It adds three things:
  ...
  - `metrics.json`, derived after the run from `events.jsonl` (codex `--json` event stream): `tool_calls` by kind, `total_tool_calls`, `retries` (a failed command re-run with the same argv), `duplicate_actions` (an identical command or file edit repeated after success), `tokens` (the sum of `turn.completed.usage`), `completion_reason` (exit/usage-limit/no-output code), and `skills_read` (`SKILL.md` paths the subject opened).
...
- **Decision**: vault resolution order is `--vault` > `OBSIDIAN_VAULT_PATH` > repo `.env` > `<repo_root>/wiki` if it exists (from `pyproject.toml` `[tool.agentic-co-dm] vault`) > `~/.obsidian-wiki/config`. Repo root comes from the script location, not the working directory. `lint fix` becomes an argparse subparser under `lint`. The positional `fix` token is kept as a deprecated alias that behaves identically, so existing docs keep working, and is removed from the docs. The full contract is in `contracts/wiki-cli.md`.
...
- **Side effect observed**: running lint with Vale rewrote the tracked `styles/config/vocabularies/CoDM/accept.txt` (via `tools/creative_lint/vale_vocab.py`) and `wiki/_meta/lint-cache.json`. Both were reverted after measuring. The vocabulary regeneration is existing behavior and is noted for Clarify (plan.md spec gap 6).
