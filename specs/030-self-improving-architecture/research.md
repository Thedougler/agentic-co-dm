...
- **Decision**: `scripts/luna-eval` stays the only runner. It adds three things:
  ...
  - `metrics.json`, derived after the run from `events.jsonl` (codex `--json` event stream): `tool_calls` by kind, `total_tool_calls`, `retries` (a failed command re-run with the same argv), `invocation_errors` (FR-027 command calls rejected as usage errors, counted even when corrected; data-model §3; the SC-010 measure), `duplicate_actions` (an identical command or file edit repeated after success), `tokens` (the sum of `turn.completed.usage`), `completion_reason` (exit/usage-limit/no-output code), and `skills_read` (`SKILL.md` paths the subject opened).
