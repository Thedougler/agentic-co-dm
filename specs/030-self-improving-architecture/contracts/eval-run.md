...
- **Runner**: `scripts/luna-eval --skill <skill-dir> --eval <id> --out <iteration-dir> [--config with_skill|without_skill|old_skill] [--subject-skill <dir>] [--run n]`. The flags stay as they are today. Added: repo-root discovery, `metrics.json` (data-model.md §3, including `retries` and `invocation_errors`), and the `skill_selected` grade written into `grading.json`.
