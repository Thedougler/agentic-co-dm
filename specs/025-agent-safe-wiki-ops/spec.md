...
- Q: Should Vale custom rules run as a pre-pass feeding into the existing Python lint finding schema, or as a standalone layer? → A: Vale findings feed into the existing Python lint finding schema (unified pipeline). One finding format; deprecated-pattern detections get the same dry-run/repair/transaction treatment as every other lint finding.
- Q: Should deprecated-pattern rules match only headings, or also inline content and other surfaces? → A: Headings and inline content (Vale's natural scope); frontmatter keys stay with Python checks. Additionally, AI tells and other community Vale packages (e.g., write-good, proselint) MUST be included and flagged by the linter alongside custom deprecated-pattern rules.
- Q: Should deprecated-pattern findings be deterministic-repair, human-repair, or per-rule? → A: All deprecated-pattern findings are deterministic agent repair, never human repair. The agent acts on the guidance in the rule definition without waiting for DM review.
...
- **SC-008**: A default `wiki-lint` run exposes a positive 1-based source line for every file-scoped structural, template, and Vale finding in both the primary findings and `findings_by_file` output.
