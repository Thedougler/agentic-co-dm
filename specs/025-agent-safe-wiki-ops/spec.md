...
4. **Given** a consolidation merges two faction pages, **When** the merge completes, **Then** the canonical page, redirect, index entry, manifest identity, and backlinks update in one logical operation — no manual regex editing.
5. **Given** a multi-file repair transaction completes, **When** finalization runs, **Then** QMD, index, and manifest refresh exactly once (not after each intermediate write).
...
4. **Given** a rename_or_merge_page mutation, **When** it executes, **Then** the canonical page, redirect, index entry, manifest identity, and deterministic backlinks all update in one atomic operation.
...
- **FR-001**: System MUST provide an identity-resolution layer that classifies wiki pages as `resolved`, `ambiguous`, or `distinct` using canonical path, title, aliases, redirects, manifest provenance, type/kind, merge history, and content overlap signals.
...
- **FR-010**: rename_or_merge_page MUST atomically update canonical page, redirect, index entry, manifest identity transition (merged_into), and deterministic backlinks.
...
- **FR-016**: Repeated agent operations MUST have one canonical command surface with consistent arguments, --help, JSON mode, explicit exit meanings, and actionable error messages.

### Key Entities

- **Page Identity**: Canonical path, title, aliases, redirects, merge history, type/kind — the resolved identity of a wiki page.
...
- **SC-005**: A two-page faction merge (canonical + redirect + index + manifest + backlinks) completes as one logical operation without intermediate QMD refreshes.
...
- **SC-007**: Every open error in errors.md (e-10, e-12 through e-21, e-23) has a causal regression test that fails before the fix and passes after.
...
- Identity resolution uses signals already present in the repository (frontmatter, filenames, manifest, redirects, wikilinks) — no new external services or embeddings.
