...
**Primary Dependencies**: One new project dependency: **Vale** (prose linter, installed and available by default). Existing stack: Python stdlib, QMD CLI/content index for similarity, PyYAML already used by 024-creative-linting, `scripts/wiki-bulk-ops` (frontmatter parsing, link rewriting, atomic writes, rename, replace, tag normalize), `tools/lint_wiki.py` (structural HARD checks), `scripts/manifest.py` (manifest CRUD), `scripts/wiki-lint` (CLI wrapper), `scripts/wiki-maintain` (Layer A facade). Template files at `wiki/templates/*.md`. 024-creative-linting adds `tools/creative_lint/` (registry, engine, findings, severity, bundles) and `rules/` (registry.yml, bundles.yml).
...
| VII. Creative Judgment Is Protected | PASS | Structural repairs (links, frontmatter, index entries) are deterministic. Vale prose-quality findings (AI tells, weasel words, deprecated patterns) are deterministic agent repairs — the agent rewrites to meet quality standards, which is corrective maintenance per Constitution XV/XIX, not creative judgment override. Repair classes explicitly separate deterministic from judgment-only. |
...
.vale.ini                 # New: Vale config (points to styles dir, scopes wiki/)
styles/
├── Deprecated/           # New: custom deprecated-pattern rules
│   └── DMThesis.yml      # First banned pattern — junk section agents hallucinate
├── AITells/              # New or community: AI-generated prose detection
├── write-good/           # Community Vale package
└── proselint/            # Community Vale package
...
**Structure Decision**: New mutation/transaction logic goes in `tools/wiki_ops/` as a library consumed by CLI scripts. CLI surfaces extend existing `scripts/wiki-bulk-ops` and `scripts/wiki-lint` with new subcommands, plus one new `scripts/wiki-identity` for identity resolution. Template contracts are YAML files under `wiki/templates/contracts/`. Vale config (`.vale.ini`) lives at repo root; custom styles in `styles/Deprecated/`, community packages in `styles/`. A Python adapter (`tools/wiki_ops/vale_adapter.py`) runs Vale, parses its JSON output, and converts findings to the standard `LintFinding` schema with `repair_class: deterministic_repair`. This preserves the existing pattern: library code in `tools/`, CLI entry points in `scripts/`, tests in `tests/`.
...
| XVI. The Simplest Adequate Tool | PASS | Reuses `wiki-bulk-ops` frontmatter parsing, link rewriting, and atomic writes. stdlib hashlib for preconditions. Existing QMD provides content similarity. One new dependency: Vale (purpose-built prose linter with community packages for AI tells, write-good, proselint — these are well-maintained packages that replace writing custom detectors from scratch). |
...
**Post-design gate**: PASS. No `NEEDS CLARIFICATION` remains. All five layers (identity, mutation, scope, transaction, Vale prose/pattern) compose through existing CLI patterns. Template contracts are YAML co-located with templates. Vale adapter converts findings into the unified finding schema. Error regression tests cover all 28 open errors in SC-007.
