...
**Testing**: [quickstart.md](./quickstart.md). Classify the contract jobs. Observe dispatch vs session-agent vs parked. No bulk suite. No live writer required to prove classification.
...
**Constraints**: FR-002 length is not the gate. FR-008 routing fires without loading a skill-authoring playbook. FR-013 closed in-scope set. FR-017 usage-limit parks include a retry time and are not retried before it. Constitution VII — do not prescribe how the designated writer designs. Constitution IX — standing gate stays short; procedure disclosed. Constitution X — do not ask the owner to classify, park, or commit.
...
| II. Issues are the work surface | Pass — parked dispatch is a GitHub issue via `gh`, label `ready-for-agent`. No new triage label. PRs are not a request surface. |
...
**Post-design re-check**: still pass. Contract is classification of an in-scope edit plus the dispatch outcome. Complexity table empty.
