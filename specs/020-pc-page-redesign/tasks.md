...
- [ ] T004 [P] [US1] Using `.agents/skills/obsidian-layout-adjustment/SKILL.md`, remorph PC-page scan tweaks in `wiki/.obsidian/snippets/pc-sheet.css` scoped to `.pc-sheet` (featured-portrait size, three-column sheet widths, heading density); keep vault-wide defaults in `wiki/.obsidian/snippets/ttrpg-styles.css` and `wiki/.obsidian/snippets/wide-note-surface.css`; MUST NOT add a distinct decorative theme, cards, site colors, or D&D Beyond chrome; MUST NOT use markdown `flexGrow` unless CSS cannot set column widths
