...
- Q: Where should the one featured picture of the player character sit on the page? → A: Option C — After the player-safe look, portrait and Identity side by side in columns. Extra pictures may sit next to the section they illustrate; leftover pictures go in an Art gallery at the bottom. One picture only → featured portrait, no gallery.
- Q: When a player's stated goal is written onto their character page, must both the session transcript and the session summary support it? → A: Option B — Transcript alone is enough if the player clearly said it; the summary is optional backup.
- Q: Besides default styling for the whole vault, what extra CSS should player-character pages get? → A: Option A — Vault-wide default CSS plus PC-page readability tweaks only (portrait size, column widths, sheet density). No decorative PC theme or card chrome.
- Q: Which table statements count as a player's stated goal on the character page? → A: Option A — Both in-character and out-of-character player statements from the transcript. Do not infer from actions.
- Q: When should Stated Goals on a player-character page be refreshed from session transcripts? → A: Option A — After every session that included that player, refresh from that session's transcript. Add newly stated goals; drop a goal only if the player said it is done or abandoned.
- Q: Should every player-character page keep the same D&D Beyond sheet sections — including Spells — even when that character has no spells or an empty subsection? → A: Option A — Every page always has Identity, Combat Stats, Ability Scores, Skills, Actions, Spells, Inventory, and Features in that order. Spells stays even with no casting (state none). Connections, Stated Goals, Session Log, and Art still drop when empty.
- Q: How should a player-character page use Obsidian columns so it scans like D&D Beyond? → A: Option C — Three columns like D&D Beyond: abilities | skills/saves | actions/spells/inventory/features.
- Q: What happens with blank or broken images on a PC page? → A: Blank images are omitted. Linked images must resolve. Images belong on the PC pages.
...
A DM opens a player character page during preparation or play. The page gives them an immediate player-safe look, party orientation, combat reference, key abilities, current resources, useful inventory, decision-relevant connections, and — when present — the player's stated goals, without requiring a search through satellite pages or source dumps. The players make all decisions and run their characters; the page helps the DM understand, portray, adjudicate their choices, and plan later sessions with that player.
...
**Independent Test**: Open each of the five current PC pages and, without opening another page, identify the character's player, class and level, current combat resources, signature actions, key relationships, decision-relevant pressures, and any stated goals in under one minute.

**Acceptance Scenarios**:

1. **Given** a complete PC page with at least one picture of the character, **When** the DM opens it, **Then** the page begins with one title, a player-safe narration block, and a D&D Beyond-style Identity header paired in columns with one featured portrait, containing the player's identity, class and level, and other identity facts when known.
...
5. **Given** a PC page with more than one picture of the character, **When** the DM reads past Identity, **Then** additional pictures appear next to the section they illustrate when relevant, and every leftover picture appears only in the Art gallery at the bottom.
6. **Given** a PC whose player clearly stated a goal in a game-session transcript, in character or out of character, **When** the DM opens the page to plan a later session with that player, **Then** Stated Goals lists that goal and does not list inferred or summary-only goals.
...
4. **Given** a PC with no spells, **When** the page is filed, **Then** Identity, Combat Stats, Ability Scores, Skills, Actions, Spells, Inventory, and Features still appear in that order. Spells states that the character has no spells rather than omitting the heading. Unused campaign extras (Stated Goals, Art, and Session Log or Connections when empty) are still omitted.
...
8. **Given** a session transcript in which the player clearly stated a goal in character or out of character, **When** the PC-owning skill updates that character's page, **Then** it records the goal under Stated Goals. A session summary MAY help locate or paraphrase it but MUST NOT add a goal the transcript does not support.
9. **Given** no game-session transcript in which the player clearly stated a goal, **When** the page is filed or updated, **Then** Stated Goals is omitted rather than filled from inference, connections, or a summary alone.
10. **Given** a completed session that included the player, **When** Stated Goals is refreshed from that session's transcript, **Then** newly stated goals are added, a goal is removed only if the player said it is done or abandoned, and inferred goals are still omitted.
...
3. **Given** an Obsidian Reading view, **When** the DM opens a conformed page, **Then** D&D Beyond-style scan surfaces render through supported `col` / `col-md` codeblocks: featured portrait beside Identity when art exists; Combat Stats as a full-width row; then three columns (Ability Scores | Skills and saves | Actions, Spells, Inventory, and Features stacked). Narration remains a real callout outside those fences, and the document still has a meaningful linear reading order if columns are unavailable.
4. **Given** Obsidian Reading view with vault CSS enabled, **When** the DM opens a conformed PC page and any other wiki page, **Then** vault-wide default CSS applies everywhere, and the PC page has extra scan tweaks for the sheet (portrait size, column widths, heading density) without a separate decorative theme, cards, or D&D Beyond chrome.
...
- A PC has no class, racial, feat, or other spell access: the Spells heading remains and states none; the page does not omit Spells.
...
- A PC has no current session change, named connection, or transcript-supported player-stated goal: omit that optional section rather than leaving an empty heading.
- A PC has no pictures of the character: Identity has no featured portrait and the Art gallery is omitted.
- A PC has exactly one picture of the character: that picture is the featured portrait paired with Identity; the Art gallery is omitted.
- A PC has leftover pictures after the featured portrait and any in-section placements: those leftovers appear only in the Art gallery at the bottom.
...
- A blank or unresolved image embed: omit it. Do not leave an empty portrait column or a dead image link.
- Linked images on a PC page must resolve to an existing vault file. Character pictures that exist must appear on the PC page (featured portrait, in-section, or Art).
...
- A session summary mentions a goal that the transcript does not show the player stating: do not add it to Stated Goals.
- In-character and out-of-character player statements both count when the transcript shows the player said them; implied wants from actions do not.
- A goal is only implied by play, connections, or DM thesis: do not add it to Stated Goals.
- After a session that did not include the player: do not refresh that PC's Stated Goals from that session.
- A previously listed goal is dropped only when the player said in a later transcript that it is done or abandoned; silence is not abandonment.
...
- **FR-002**: The PC template MUST define a stable, single-H1 section spine that follows D&D Beyond information groups and order: player-safe spoken look, Identity paired with one featured portrait when art exists, Combat Stats, Ability Scores, Skills, Actions, Spells, Inventory, Features, then campaign extras Connections, conditional Stated Goals, Session Log, and conditional Art gallery. Identity through Features MUST appear on every filed page. Unused campaign extras MUST be omitted from filed pages.
- **FR-003**: Identity MUST expose player, class and level, and other D&D Beyond identity facts when known (such as species/race and background). When at least one picture of the character exists, Identity MUST be paired in supported columns with one featured portrait after the player-safe look. Home ship or equivalent current base, when known, MAY appear in Identity or Connections and MUST NOT be dropped. Identity MUST NOT require a DM thesis or prescribed play pattern.
...
- **FR-006**: Every filed PC page MUST include a Spells section in the D&D Beyond sheet order. Spells MUST cover class casting, racial spells, feat spells, and other spell-like access when present, and MUST distinguish spellcasting ability and calculations, cantrips, prepared or known spells, and slots or other casting resources when those apply. A character with no spell access MUST still have the Spells heading and MUST state that they have no spells rather than omitting the section.
...
- **FR-010**: The template and applicable PC guidance MUST use the repository's supported Obsidian column codeblock syntax (`col` and `col-md`) to render D&D Beyond-style rows and columns: featured portrait beside Identity when art exists; Combat Stats as a full-width row; then three columns — Ability Scores | Skills and saves | Actions, Spells, Inventory, and Features stacked. Narration callouts MUST stay outside those fences. Headings MUST remain in a readable linear order when columns do not render. The page MUST NOT attempt a visual clone of D&D Beyond chrome (cards, site colors, or non-Markdown layout).
...
- **FR-021**: Character images MUST appear on the PC page. One featured portrait after the player-safe look is paired with Identity in columns; additional pictures MAY appear next to the section they illustrate when relevant; remaining unused pictures MUST appear only in the Art gallery at the bottom. If only one picture exists, it MUST be the featured portrait and the Art gallery MUST be omitted. If no pictures exist, both the featured portrait and the Art gallery MUST be omitted. Blank image embeds MUST be omitted. Every image wikilink or embed on the page MUST resolve to an existing vault file.
- **FR-022**: Stated Goals MUST list only goals the player clearly stated in a game-session transcript, whether spoken in character or out of character at the table. A session summary MAY be used as optional backup to locate or paraphrase a transcript-supported goal and MUST NOT be the sole source. The section MUST be omitted when no such player-stated goal exists. Agents MUST NOT invent, infer, or prescribe goals from play patterns, connections, or DM thesis. When the section is present, session planning with that player MUST use it. After every session that included that player, Stated Goals MUST be refreshed from that session's transcript: add newly stated goals; remove a goal only if the player said it is done or abandoned.
- **FR-023**: The vault MUST apply default CSS vault-wide. Player-character pages MUST receive additional CSS tweaks that improve scan and readability of the sheet (including featured-portrait size, column widths, and heading density). Those PC-page tweaks MUST NOT introduce a distinct decorative theme, cards, site colors, or D&D Beyond chrome. CSS MUST NOT be the only way to understand the page; headings and tables remain the semantic fallback.
...
- **Identity**: The D&D Beyond-style header for player, class and level, and other identity facts when known. When art exists, it is paired in columns with one featured portrait after the player-safe look.
...
- **Featured portrait**: The single top-of-sheet picture of the player character, paired with Identity; it is not a second live owner and is omitted when no pictures exist.
- **Art gallery**: The optional bottom section that holds leftover pictures of the character after the featured portrait and any in-section placements; omitted when none remain.
- **Stated Goals**: The optional campaign section listing goals the player clearly stated in a game-session transcript, in character or out of character. Session summaries are backup only. Omitted when none exist. Refreshed after every session that included that player. Used when planning later sessions with that player.
...
- **SC-006**: 100% of filed PC pages include Identity, Combat Stats, Ability Scores, Skills, Actions, Spells, Inventory, and Features in that order. Spells is present even when the character has no spells. Unused campaign extras (Connections, Stated Goals, Session Log, Art) are absent when their source contains no applicable content; no conformed page contains an empty optional campaign heading or placeholder table.
- **SC-007**: In an Obsidian Reading view, the primary PC sheet surfaces render as D&D Beyond-style rows and three columns using the supported codeblock syntax (featured-portrait and Identity pair when art exists; Combat Stats full-width; Ability Scores | Skills and saves | Actions/Spells/Inventory/Features), while the same page remains understandable in linear Markdown when column rendering is unavailable.
...
- **SC-010**: 100% of Stated Goals entries on conformed PC pages are supportable from a game-session transcript as the player's own words, in character or out of character; 0% are inferred or summary-only. Pages with no such transcript evidence omit the section.
- **SC-011**: In Obsidian Reading view, vault-wide default CSS is active on non-PC pages, and each of the five conformed PC pages shows additional sheet-scan tweaks without a distinct PC visual theme. With CSS disabled, the same pages remain understandable from headings and tables.
- **SC-012**: After a session that included a given player, that PC's Stated Goals includes every newly transcript-stated goal from the session and retains prior goals unless the player said they are done or abandoned. After a session that did not include the player, that PC's Stated Goals is unchanged.
...
- Stated Goals are sourced from game-session transcripts as the player's own in-character or out-of-character words; session summaries are optional backup and not an independent source. The feature does not invent player goals. Refresh is after every session that included that player.
- Vault CSS uses the existing Obsidian snippet mechanism. PC-page tweaks are additive scan/readability rules, not a second theme.
