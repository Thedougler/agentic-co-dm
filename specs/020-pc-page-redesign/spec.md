...
3. **Given** an Obsidian Reading view, **When** the DM opens a conformed page, **Then** D&D Beyond-style paired scan surfaces render through supported `col` / `col-md` codeblocks including the featured-portrait and Identity pair, narration remains a real callout outside those fences, and the document still has a meaningful linear reading order if columns are unavailable.
...
- **FR-010**: The template and applicable PC guidance MUST use the repository's supported Obsidian column codeblock syntax (`col` and `col-md`) to render D&D Beyond-style paired scan surfaces, including the featured-portrait and Identity pair, keep narration callouts outside those fences, and preserve a readable linear order. The page MUST NOT attempt a visual clone of D&D Beyond chrome (cards, site colors, or non-Markdown layout).
...
- **FR-021**: Generated art of the character MUST be used on the PC page as follows: one featured portrait after the player-safe look, paired with Identity in columns; additional pictures MAY appear next to the section they illustrate when relevant; remaining unused pictures MUST appear only in the Art gallery at the bottom. If only one picture exists, it MUST be the featured portrait and the Art gallery MUST be omitted. If no pictures exist, both the featured portrait and the Art gallery MUST be omitted.
...
- **SC-007**: In an Obsidian Reading view, the primary PC sheet surfaces render as D&D Beyond-style column pairs using the supported codeblock syntax, including the featured-portrait and Identity pair when art exists, while the same page remains understandable in linear Markdown when column rendering is unavailable.
