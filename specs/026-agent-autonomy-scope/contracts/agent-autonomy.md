...
| Executable rules | `rules/registry.yml` `luna-eval` evals via `scripts/luna-eval` |
...
- Point at `luna-eval` evals / wiki-lint for the contract.
...
### `luna-eval` Work-gate evals (BLOCK)
...
### the placement rule (no current checker) (BLOCK)
...
### the spec-citation rule (no current checker) (BLOCK)

A `specs/*/spec.md` Functional Requirement that contains `agent-facing` MUST cite at least one `rules/registry.yml` id. This feature’s spec cites `luna-eval` evals. Every existing spec that contains that phrase MUST cite an id.
...
2. Constitution, AGENTS.md, and `luna-eval` evals do not disagree.
