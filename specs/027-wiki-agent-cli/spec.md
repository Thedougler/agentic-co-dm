...
- Default stdout is **agent-shaped**: one structured object, stable keys, no terminal detection. `--json` is accepted and ignored. `--pretty` is explicit human text.
...
- Default whole-wiki and scoped lint output includes aggregate counts, every flat per-file finding, cache/scope metadata, and an actionable `next` recommendation. `next.path` is the smallest dirty file by byte size, with path as the tie-breaker. `--full` is accepted as a compatibility no-op.
...
- Q: When an agent runs `wiki lint`, what should the default output be? → A: Always dump every finding, grouped by file (complete Vale-included breakdown per file with 1-based line numbers; no worklist-only default).
...
- **FR-009**: Default lint objects MUST include `status`, `counts`, `hard_fail`, `finding_total`, `affected_pages`, `next_page`, `next` (an actionable recommendation containing `path`, `findings`, `bytes`, and `action` or null), `cache` (hits, misses, Vale skipped), `files_checked`, `scope`, `ledger` (open count and ids from `errors.md`), `timing`, `unique`, `backlog`, and complete per-file `files` findings.
- **FR-010**: Default `wiki lint` output MUST include aggregate counts and every finding for the scoped files, including every Vale finding and every severity, grouped by file. Each file group has the file path and flat records with `rule`, `file`, `line` (1-based), `severity`, and `message`. Nested per-rule finding maps MUST NOT be used.
- **FR-011**: `--full` MUST be accepted on lint and MUST return the same complete result as the default. It MUST NOT suppress or add findings, and lint MUST NOT provide a summary-only omission mode.
...
- **FR-018**: Agent instructions that tell agents how to lint, query, or health-check the wiki MUST be updated to this command, to the complete default lint result, and to: run `wiki health`, then act on `context.act` (first-turn and skill load) then `next` (then remaining `focus`) without terminal detection, extra interpretation, or waiting for the DM. For a lint work item, agents MUST run the deterministic `wiki lint fix` sweep once, then manually repair each remaining file with `wiki lint <next.path>` before proceeding.
...
- **Worklist**: Complete default lint result — aggregate counts, every finding grouped by file, cache/scope metadata, and `next` for the smallest dirty file; `--full` is a compatibility no-op.
...
- **SC-001**: An agent lints a directory or the whole wiki with one invocation and receives complete aggregate issue counts, every finding, and an actionable `next.path`; it can begin the next repair without another detail-only lint mode.
- **SC-002**: Default lint of two named files, a prefix, or the whole wiki returns complete Vale-included findings for that scope with 1-based line numbers, complete issue counts, and the smallest dirty file. `--full` returns the same result.
...
- **SC-008**: Independent agent given only this spec’s command examples lints the whole wiki, names aggregate issue counts and `next.path` with its action, and uses the default finding dump to repair the selected file.
...
- **SC-017**: Agent instructions and command examples name `wiki lint fix <next.path>` as the first repair step and `wiki lint <next.path>` as the manual one-file follow-up for findings that remain.
...
- All configured checkers, including structural, template-conformance, creative, and Vale checks, run through the agent-facing `wiki lint` command. Its default result includes every finding regardless of severity. Agents do not need checker-suppression flags.
- `--full` is accepted as a compatibility no-op; it does not suppress or add findings. The agent-facing lint command has no `--all`, `--no-vale`, or `--no-template` flags.
...
- Agent-facing guidance MUST present `wiki lint fix <next.path>` before `wiki lint <next.path>`; the latter is the manual one-file follow-up for remaining findings.
