...
One `scripts/wiki` command (`lint`, `query`, `health`) that composes existing structural lint, Vale, Layer A maintenance, sitting/efficiency/error trackers, and qmd retrieval. Default stdout is a compact worklist/snapshot agents can load (`health` includes `trends`, ordered `focus`, and `next`); `--pretty` is the only human text; `--json` is a no-op. Unchanged files reuse checker results from `$VAULT/_meta/lint-cache.json`. Creative lint stays on `scripts/wiki-lint`. `scripts/wiki-maintain --report` aliases `wiki health`.
...
**Storage**: Wiki markdown on disk. Checker cache: `$VAULT/_meta/lint-cache.json` (lint already skips `_meta`). Not git-canonical wiki content. Health **reads** `sittings.jsonl`, `errors.md`, `.local/efficiency/traces.jsonl`; it does not write them.

**Testing**: pytest temp-vault pattern in `tests/test_wiki_ops.py`. New `tests/test_wiki_cli.py`. Live-wiki size check for SC-002. Cold-context smol subject for SC-008 and SC-009.
...
**Scale/Scope**: One dispatcher script, thin worklist/cache/pretty/health helpers under `tools/wiki_ops/`, health alias, skill/AGENTS command-string updates. Does not replace `tools/lint_wiki.py` or invent a second telemetry store.
...
| III. Spec Before System Change | PASS | [spec.md](spec.md) has FR/SC; grill + clarify 2026-09-19 locked health trends/`focus`/`next`. |
...
| IX. Measured Efficiency | PASS | Worklist + cache; drop nested finding dumps and health essays; trends are compact aggregates. |
...
| XIV. Simplest Adequate Tool | PASS | Compose existing checkers and trackers; one new `scripts/wiki`. |
...
Same table as above. Design artifacts: [research.md](research.md), [data-model.md](data-model.md), [contracts/wiki-cli.md](contracts/wiki-cli.md), [quickstart.md](quickstart.md). No new `NEEDS CLARIFICATION`. Cache path, qmd flags, trend sources, `focus` cap 5, and `next` = `focus[0]` are resolved. Gate still PASS.
