...
Use this reference when working with the DM on changing how Obsidian looks using CSS snippets.
...
## DM Language To Obsidian Backend

the DM names the visible object. Translate it into the Obsidian object before editing.
...
| the DM might say | Usually means | Backend / selector family to inspect |
| --- | --- | --- |
...
The same visible object can require different backend work depending on what the DM asks for.
...
Save a baseline, preserve selector order, add section headers/file map, format, screenshot, and archive inactive experiments outside `vault/.obsidian/snippets`.
...
the DM's corrections are diagnostic:
...
- the DM's corrections are selector evidence.
...
Keep experiments outside `vault/.obsidian/snippets` so the snippet picker stays clean. Active snippets should be the live design, not the archive.
...
- changing anything outside `vault/.obsidian/snippets` or Obsidian settings files
