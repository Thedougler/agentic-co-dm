...
def rename_actor(old: str, new: str, profiles_dir: Path = PROFILES_DIR) -> ActorProfile:
    """Rename an actor, moving its profile directory and updating the stored name.

    Returns the renamed ActorProfile (reloaded from disk, with personas intact).
    """
    old_slug = _slug(old)
    new_slug = _slug(new)
    old_dir = profiles_dir / old_slug
    new_dir = profiles_dir / new_slug

    if not old_dir.exists():
        raise ValueError(f"Actor '{old}' not found")
    if new_slug != old_slug and new_dir.exists():
        raise ValueError(f"Actor '{new}' already exists")
    ...
    logger.info("Renamed actor: %s -> %s", old, new)

    actors = load_actor_profiles(profiles_dir)
    return actors[new_slug]


def rename_persona(
    actor: str, old: str, new: str, profiles_dir: Path = PROFILES_DIR
) -> PersonaProfile:
    """Rename a persona under an actor, moving its files and updating the stored name.

    Returns the renamed PersonaProfile (reloaded from disk, with exemplars/prosody intact).
    """
    ...
    if not (actor_dir / f"{old_slug}.npy").exists():
        raise ValueError(f"Persona '{old}' not found under actor '{actor}'")
    if new_slug != old_slug and (actor_dir / f"{new_slug}.npy").exists():
        raise ValueError(f"Persona '{new}' already exists under actor '{actor}'")

    if new_slug != old_slug:
        for suffix in (".npy", "_prosody.yaml", "_exemplars.npy", "_meta.yaml"):
            src = actor_dir / f"{old_slug}{suffix}"
            if src.exists():
                src.rename(actor_dir / f"{new_slug}{suffix}")

    persona_meta_path = actor_dir / f"{new_slug}_meta.yaml"
    persona_meta = {}
    if persona_meta_path.exists():
        with open(persona_meta_path) as f:
            persona_meta = yaml.safe_load(f) or {}
    persona_meta["name"] = new
    with open(persona_meta_path, "w") as f:
        yaml.dump(persona_meta, f, default_flow_style=False)

    actor_meta_path = actor_dir / "meta.yaml"
    actor_meta = {}
    if actor_meta_path.exists():
        with open(actor_meta_path) as f:
            actor_meta = yaml.safe_load(f) or {}
    personas = actor_meta.get("personas", [])
    actor_meta["personas"] = [new_slug if p == old_slug else p for p in personas]
    with open(actor_meta_path, "w") as f:
        yaml.dump(actor_meta, f, default_flow_style=False)

    logger.info("Renamed persona: %s -> %s (actor: %s)", old, new, actor)

    actors = load_actor_profiles(profiles_dir)
    return actors[actor_slug].personas[new_slug]
