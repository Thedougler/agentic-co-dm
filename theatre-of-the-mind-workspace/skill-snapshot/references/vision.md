...
Add pixel-supported facts to your private fact list (`SKILL.md` step 2).
Mark the source as `pixels:<path>`.
...
A portrait takes durable identity from the art (body, clothes, marks); a fight
pose or market stall in the picture stays out of it. A situated scene may use
the pose and setting when they match the table state. A painted secret, hidden
item, or unearned name still needs a way to reach the players
