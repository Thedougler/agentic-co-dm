...
Theatre of the mind owns every player-facing prose surface. Route the job by
what the player needs to do, not by whether the source note calls it a scene.
Read this file before drafting, then read [examples.md](examples.md),
[voice.md](voice.md), [boundary.md](boundary.md), and the matching specialist
reference. When related art exists, also read [vision.md](vision.md) and open
the **pixels** before drafting. Choose the surface's primary job, ownership,
interaction state, and natural stop. The contracts below are distinct; do not force a dialogue,
object, recap, handout, vision, or transition through a location-description
template.
...
- **Situated moment:** use the supplied viewpoint and current table state, then
  ...
  route, motion, and scene stock; a doorway-only slice is partial only when
  explicitly asked.
- **Standalone portrait:** write one self-contained player-reference entry for
  one subject outside a current encounter. Do not invent a party, specific
  environment, viewer or camera, current motion, interaction, dialogue,
  pressure, handoff, or closing question. Hierarchically cover the supplied
  visible identity, stable silhouette/whole and scale, defining parts or
  material, ordinary observable function or behavior when supplied, and a
  signature sensory fact when canon supplies one. Apply the subject additions in
  the top-level mode section of `SKILL.md`; do not use scene staging or a
  located change. The completeness test is recognition, picture, distinction,
...
attunement, curses, private biology/history, tactics, hidden causes, learned
route rules, or secret identities into the cold portrait. An explicit request
for identified or player-known properties grants a separate mechanics layer
after the description; it does not change the default. Portrait completeness
is contract coverage, not a dump of every parent heading.

## Contracts

| Surface | Primary job and contract | Natural stop |
| --- | --- | --- |
| Place or encounter | Use the spatial camera. Select one frame: **approach/route**, **survey/layout**, or **tactical field**. Seat the viewpoint, introduce each landmark once, and put one signature property on a usable anchor. **Scene-setting (`Initial Narration`)** is the **Layer 1 immediate frame** in second person present tense: where the party stands, body-scale size, ground/air/light, then what is obvious without deliberate investigation. Frame and body-scale environment seat the party before furniture; body-scale environment comes from the space, not assigned PC physiology. Salient features and discoverable information are separate complete reveal blocks or zone/tick Narration cells (Layers 2–3) that describe the discovery, not the method. Features that matter tactically or interactively belong in Layer 1 before a player would need them. Player-facing distance uses relational bands (Melee, Near, Far); DM-facing zones may keep feet. An owner photo on the run card is a DM glance; open those **pixels** per [vision.md](vision.md). Do not say "the crew". | Stop on a live situation (danger, contradiction, question, opportunity, demand, objective, or changing circumstance) after the Layer 1 frame gives the table a stable shared picture. Do not choose the party's response. |
...
| Vehicle | Use the spatial/entity hybrid: camera, silhouette and scale, how the vehicle occupies or moves through its environment, only a currently observable or established route/state, a supported boarding or access relation in kitchen-table language (lines, rail, hold, hatch, or another established physical access point), one operational signature, and the current affordance or handoff. Never say boarding interface in player prose. Name both ends of a relationship when needed (for example, “the crane swings over the cargo deck”). Do not infer party size or a boarding restriction; state a restriction only when current perception or established canon supports it. Source silence is not evidence of absence: do not narrate no gangway, empty dock, closed access, missing staff, or missing route unless the viewpoint/current state or parent positively establishes that absence. Do not turn this sequence into a formula or invent an interface missing from the parent. | Stop at the supported access point, operating problem, arrival, or opening the players can use; a natural handoff need not be a question. |
| Technique (`kind: technique`) | **Non-object tell contract** (not Fate Spinner gear bar). Cold `[!narration]`: breath/stance/wing set and how the form looks when used as stable facts. Ban invented venue/terrace scene, object silhouette, and “answers” mysticism. | Stop before a resolved hit; venue + package stay Fiction/Mechanics. |
| Object, item, or landmark | Cold Appearance: concrete noun, scale vs body, material+wear, one non-sight sense or ordinary behavior. Fail catalog gloss (`carries as`, `ordinary X for Y`), biography closers, rarity/DC. Held/inspected still exterior+contact only — not interior/function/history/magic. Landmark = object you keep using. | Stop at drawable object picture (cold) or affordance/handoff (situated). |
...
| Transition | Mark the time cut or movement, orient only what changed, and show the new situation and handle. Do not cut across a consequential decision or redraw an unchanged place. | Stop as soon as the new situation is oriented and the next meaningful choice is visible. |
...
- **Item:** identify the common whole or type, silhouette and scale,
  ...
  physical behaviors. A state-qualified cross-section is allowed as general
  description — “when cut open” — but never pretends an intact viewer saw
  inside. Keep Appearance/Cold Portrait, Identified Properties, and DM
  Lore/Secret separate; render identified properties only when that knowledge
...
- **Creature:** make a species identity card: stable body plan and silhouette,
  ...
  behavior (idle/species habit), and habitat only as stable ecology. Keep
  signature moves, tactics, ambush scripts, time-skip films, and resolved
  attacks out of the cold `[!narration]` block. Ordinary behavior ≠ At-the-table
  tactic script. A response state reports the observable stimulus and response,
  then stops. Do not add an unseen agent, cause, direction, meaning, threat, or
  tactical interpretation, including a hedge. Do not invent a marker from a name
  alone. See the creature-response gate in [boundary.md](boundary.md).
...
For place, encounter, travel, and vehicle work, make a private staging card:
...
- **Frame:** approach/route, survey/layout, or tactical field; choose one.
  Frame the space at body scale (ceiling, width, depth) before listing anchors.
...
Audit visible entrances and exits, retreat routes, cover, blocked paths, and
traversable hazards when they are decision-relevant. Then audit **scene stock**:
hazards, loot, monsters, NPCs, vehicles, doors, tracks, trails, tools, food,
water, clues, inscriptions, map features, weather, light, sound, smell, and
world-building signs. Keep only stock with an access channel and a use at this
surface: ask about it, inspect it, take it, fight it, avoid it, follow it, learn
...
Before delivery, name the branch you wrote. A spatial listener should know
where they are, what they can point at, and which route or pressure matters. An
object listener should know its state and next affordance. A creature listener
should know what manifested without receiving the stat block. A dialogue line
should leave an answerable turn. A recap should restore current pressure. A
handout should be useful on rereading. A vision should preserve uncertainty. A
transition should not steal a consequential decision. Every branch stops before
the next meaningful player choice and exposes no unearned truth.
