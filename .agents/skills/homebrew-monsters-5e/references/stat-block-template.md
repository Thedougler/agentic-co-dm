...
Linear layout: H1 → `[!narration]` → `## Statblock` (image then fence) → Visual reference when present → Behavior (list items `- **Label.**`) → Tactics (list items `- **Label.**`) → Art. No column wrappers.
