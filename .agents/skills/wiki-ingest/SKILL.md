...
**Recap** (`type: recap`, titled `Session NN - Recap`): file into the **same** session-number folder — `wiki/journal/sessions/<campaign-slug>/<NN>/Session NN - Recap.md` (alongside plan/beats when present). Do not use flat `wiki/journal/Session NN - Recap.md` or `…/recaps/`.
