...
**Finding locations:** the default report includes a 1-based `line` for each file-scoped structural, template, and Vale finding. Use `findings_by_file` to open the exact file and line; file-level findings use line `1`. Default output contains only actual findings; a clean run returns `status: "clean"` with empty findings and counts. Pass `--verbose` when the full zero-count check matrix is needed.
