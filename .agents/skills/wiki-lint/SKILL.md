...
**Deterministic pass:** from repo root run `./scripts/wiki-lint --json` first (vault `wiki/`). HARD fail keys: `broken_links`, `missing_frontmatter`, `bad_type`, `bad_lifecycle`, `typed_relationships`, `pc_identity_mismatch`, `title_stem_mismatch`, `illegal_basename`, `duplicate_stems` (redirect stubs with `redirects_to` are omitted from `missing_frontmatter` and `title_stem_mismatch`; journal `Session-*` stems are omitted from `title_stem_mismatch`; reserved files and mechanic allowlist links are omitted from `broken_links`; `_archive`/`_raw`/templates/`_meta` skipped as usual). Use that report. Soft keys: `snake_case_labels` (DM-visible snake_case table/list labels `{page,line,token,kind}`); `pc_tag_on_npc` (npc tagged pc without role/player signal); `snake_kebab_owner_basename` (snake_case/kebab-case owner basenames under `entities/{type}/`, `{page,stem,kind}`). Not HARD. Do not grep the vault for orphans or broken links unless a HARD finding is unclear.
