...
Treat city prep as a table interface, not an encyclopedia: arrival image,
recognized districts, deliberate-travel gazetteer, meaningful local rules,
visible pressure, and changed-state logs.

Practical inputs: Justin Alexander's urbancrawl layers and situation-based prep;
Mike Shea's monuments and secrets for city play; Kevin Crawford's sandbox
faction pressure; progress clocks from Blades in the Dark; campaign-status
practice that tracks current deltas instead of rewriting the whole setting.
...
City-local posture only. Per faction: public position, local objective,
