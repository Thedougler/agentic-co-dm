...
5. Invoke the designated writer with the scoped prompt: `claude -p --model claude-opus-5-5 --effort high`. Flags from `claude --help`. Do not use the `opus` alias or default Opus. Tell the writer to follow writing-for-agents and to return a commit or patch. Do not prescribe skill-design method, voice, or structure.
