...
| PC | Player-safe narration; featured portrait + Identity when art exists, else Identity full-width; Combat Stats full-width; Ability Scores; Skills; Actions; Spells when the PC can cast; Inventory; Features; Connections; Stated Goals when a transcript-supported player statement exists; Session Log; leftover Art. `cssclasses` must include `pc-sheet`. No Voice / At a Glance / Sheet / Combat Profile / Abilities / required DM thesis. Player controls the PC. Omit empty optional sections. Flatten satellites into canonical homes. Structure-only conformance does not change `lifecycle` / `reveal` / `visibility` / campaign facts. Live owner `wiki/entities/pc/` with `type: pc`. Copy `wiki/templates/pc.md`. Pass is those jobs. |
