---
title: "Sorin Vask"
aliases:
  - Sorin Vask
  - Vask's Reclaimed Goods
category: entities
tags: [shattered-sea, npc]
sources:
  - "campaign-os:sorin-vask.md"
created: 2026-09-13
updated: 2026-09-13
type: npc
reveal: unrevealed
campaign: shattered-sea
visibility: dm
summary: "Runs a dockside salvage-goods trade in Kalowe, a straight dealer within a crooked port who has drawn pointed questions from one of the dock bosses."
provenance:
  extracted: 0.85
  inferred: 0.10
  ambiguous: 0.05
tier: supporting
---
# Sorin Vask

**Wants:** to keep his trade running without any dock boss deciding it's worth a claim. Right now, one of them has started asking pointed questions about where his latest haul came from, the attention that gets a shop "protected" whether he wants it or not.

**Quote:** *Sorin Vask*: I'll tell you what it's worth. I won't tell you where it's been.

> [!narration] Narration
> Built more for hauling crates than minding a counter, broad through the shoulders and forearms scarred from years of rope and rust. Salt and tar hang thick around the stall, gulls crying from the pilings outside. He weighs a barnacled cutlass once in his palm and sets it down flat on the counter without a word. The goods sell themselves.

**Roleplay Concept:** dockside salvage foreman + the one honest fence in an outlaw port.

**Lore Sheet:** runs the shop he keeps on Kalowe's docks, trading in magic items pulled from wrecks, prize ships, and unclaimed cargo passing through. He asks no questions about where goods came from, standard practice here, but prices by real recovery risk and rarity, which keeps every dock boss unable to accuse him of cheating, only of being annoyingly honest about what a thing is worth. He keeps no ledger of who sold him what, protecting his sellers and himself in the same move. One of the dock bosses has started asking pointed questions about his latest haul.

**Toy Chest**

| Field | Value |
|---|---|
| Primary goal | Keep his trade running without any dock boss deciding it's worth a claim. |
| Consistent method | Buys quiet and sells firm-but-fair. Keeps no ledger of who sold him what, protecting his sellers and himself in the same move. |
| Active problem | One of the dock bosses has started asking pointed questions about where his latest haul came from, the attention that gets a shop "protected" whether he wants it or not. |
| Performance hooks | Dockside-salvage-foreman vibe. Weighs a piece once in his palm and sets it down flat before naming a price, never twice. |
| Link of relevance | A plausible standing fence and appraiser for whatever salvage crosses his counter next. |

**Voice & Delivery:**

- Flat, unhurried, pricing before he's finished looking. *Sorin Vask*: I'll tell you what it's worth. I won't tell you where it's been.
- *Sorin Vask*: *(setting the piece down)* That's the number. It doesn't move.
- Never posts a price twice. The weighing gesture from the Toy Chest above does the haggling for him.


## Connections

- The shop he runs on Kalowe's docks
- [[Kalowe]], the free port where he trades, watched by the Island Council's dock bosses
