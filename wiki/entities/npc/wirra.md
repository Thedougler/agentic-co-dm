---
title: "Wirra"
category: entities
tags: [shattered-sea, npc]
sources:
  - "campaign-os:wirra.md"
created: 2026-09-13
updated: 2026-09-20
type: npc
lifecycle: proposed
lifecycle_changed: "2026-09-13"
reveal: unrevealed
campaign: shattered-sea
status: alive
role: contact
location: Kalowe
faction: five-blades
visibility: dm
summary: "A precise, unsentimental Moucheron medic for the Five Blades who keeps client crews alive and charges upfront."
provenance:
  extracted: 0.85
  inferred: 0.10
  ambiguous: 0.05
base_confidence: 0.55
tier: supporting
---
# Wirra

````col
```col-md
flexGrow=2
===
## At a Glance

| **Role** | Contact |
| --- | --- |
| **Nature** | Moucheron mercenary medic |
| **Home** | [[Kalowe]] with [[five-blades|the Five Blades]] |
| **Wants** | An upfront fee and a living client crew |

> **DM thesis:** Wirra turns post-fight survival into a clear contract: pay first, then let the precise medic work.
```

```col-md
flexGrow=1
===
> [!narration] Wirra
> Wirra keeps a medical kit close and studies a wounded crew with precise, unsentimental attention. She names her fee before she names the treatment, then begins the work only after the terms are clear.
```
````

## Running Wirra

````col
```col-md
flexGrow=1
===
### First meeting

The [[five-blades|Five Blades]] call on Wirra when a job has gone bad. She checks the wounded, states the fee upfront, and leaves the client to decide whether the crew's survival is worth the price. A usable table line is: “Fee first. Then I stop the bleeding.”
```

```col-md
flexGrow=1
===
### When posture changes

Wirra's welcome ends when a client refuses the upfront fee or treats medical work as part of the combat contract. She makes the terms explicit again and may stop negotiating rather than keep a crew alive on an unclear promise.
```
````
### Voice

Wirra uses precise, unsentimental terms and avoids treating medicine as charity. Her ask is, “Fee first. Then I stop the bleeding.” Her refusal is, “No payment, no treatment.” Under pressure, she says, “Choose quickly; the wound is not waiting for your contract.”


## Connections

| Connection | Meaning |
| --- | --- |
| [[five-blades\|The Five Blades]] | Wirra is the company's medic and is called when jobs go badly. |
| [[Kalowe]] | The Five Blades work out of Kalowe, where clients can hire them. |
| [[moucheron\|Moucheron]] | Wirra's species and the blood-economy context behind the company's contracts. |
