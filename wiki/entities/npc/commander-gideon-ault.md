---
title: "Commander Gideon Ault"
aliases:
  - Commander Gideon Ault
category: entities
tags: [shattered-sea, npc]
sources:
  - "campaign-os:commander-gideon-ault.md"
  - "wiki/entities/vehicle/hcs-warrant.md"
summary: "Commander of the HCS Warrant with sealed authority to detain ships, seize cargo, arrest crews, and issue privateer letters."
provenance:
  extracted: 0.9
  inferred: 0.1
  ambiguous: 0.0
reveal: unrevealed
campaign: shattered-sea
status: alive
role: rival
location: "[[hcs-warrant]]"
faction: "[[dravosi-crown]]"
visibility: dm
created: 2026-09-13
updated: 2026-09-20
type: npc
tier: supporting
---
# Commander Gideon Ault

````col
```col-md
flexGrow=2
===
## At a Glance

| **Role**   | Rival |
| ---------- | ------ |
| **Nature** | Crown enforcement commander |
| **Home**   | [[hcs-warrant]] |
| **Wants**  | To enforce the Crown's sealed orders and control ships, cargo, crews, and privateer authority. |

> **DM thesis:** Ault is the Crown's lawful pressure at sea: he can talk, but his orders turn refusal into detention, seizure, or arrest.
```

```col-md
flexGrow=1
===
> [!narration] Commander Gideon Ault
> The HCS Warrant comes downwind and close. Commander Gideon Ault speaks from a position backed by twelve gun ports: the offer is conversation, but the ship has already made the threat legible.
```
````

## Running Commander Gideon Ault

### First meeting

Ault opens with the HCS Warrant's sealed authority. He can inspect a ship, seize cargo, arrest crew, and issue privateer letters to allies; he is ready to talk before he uses those powers. Sample line: “You may explain the cargo before I write the order that takes it.”

### When posture changes

Ault shifts from negotiation to enforcement when a crew refuses inspection, hides cargo, or obstructs a Crown order. The HCS Warrant then closes distance and makes detention, seizure, or arrest the immediate consequence.
### Voice

Ault speaks in formal, measured orders and frames coercion as procedure. He names the sealed authority before he names the consequence, offers conversation only once, and avoids admitting that the order may be politically motivated. **Ask:** “Will you explain the cargo before I inspect it?” **Refusal:** “The seal does not require your agreement.” **Under pressure:** “Then I write detention, and the Warrant makes the rest clear.”

## Connections


| Connection | Meaning |
| ---------- | ------- |
| [[hcs-warrant]] | Ault commands the Crown vessel and exercises his sealed authority from it. |
| [[uncertainty]] | The missing HCS Surety may trigger a Dravosi Crown response led by Ault; this remains a possible response, not settled fact. |
| [[dravosi-crown]] | Ault acts as Crown enforcement and may issue privateer letters to allies. |
