---
title: "Mizobasho Red Tail"
category: entities
tags: [shattered-sea, npc]
sources: ["Tokage-Island.md"]
created: 2026-09-20
updated: 2026-09-20
type: npc
reveal: unrevealed
campaign: shattered-sea
status: alive
role: contact
location: "[[Akai Hodo Village]]"
faction: "[[akai-hodo-clan|Akai Hodo Clan]]"
visibility: dm
summary: "Dark grey-scaled Eurasian lizardfolk leader of the Akai Hodo's northern marsh patrols."
provenance: {extracted: 1.0, inferred: 0.0, ambiguous: 0.0}
tier: supporting
---
# Mizobasho Red Tail

````col
```col-md
flexGrow=2
===
## At a Glance

| **Role** | Contact |
| --- | --- |
| **Nature** | Leader of the Akai Hodo Clan |
| **Home** | [[Akai Hodo Village]] |
| **Wants** | Protect Tokage's northern marshes through concealment and stealth. |

> **DM thesis:** Mizobasho Red Tail makes the red mist and hidden patrols part of the island's defense.

```

```col-md
flexGrow=1
===
> [!narration] Mizobasho Red Tail
> Mizobasho Red Tail has dark grey scales and the form of a Eurasian lizardfolk. The northern marsh's red mist and thick forest provide the natural backdrop for a leader who specializes in not being seen.
```
````

## Running Mizobasho Red Tail

````col
```col-md
flexGrow=1
===
### First meeting

The party meets Mizobasho only after reaching the Akai Hodo's marsh territory or drawing the clan's attention there.

```

```col-md
flexGrow=1
===
### When posture changes

Mizobasho's posture changes when a foreign threat reaches the island or when someone leaves evidence that the clan cannot ignore.
```
````
### Voice

Mizobasho speaks in quiet, precise observations and avoids explaining the clan's hidden routes. His ask is, “Stay where the marsh can see you.” His refusal is, “You will not take that path.” Under pressure, he says, “The mist hides us because we choose what it reveals.”

## Connections

| Connection | Meaning |
| --- | --- |
| [[akai-hodo-clan\|Akai Hodo Clan]] | Mizobasho leads the northern stealth clan. |
| [[Akai Hodo Village]] | Hidden marsh base. |
| [[Totemo Tokage]] | Mizobasho patrols the island's northeast and northwest. |
