---
title: Crissdalynn Khinriss
aliases:
  - Crissdalynn Khinriss
category: entities
tags: [shattered-sea, pc]
sources:
  - "/Users/nick/Documents/ai-co-dm/campaigns/shattered-sea/vehicles/Red Lady - Dead Lady.md"
  - "Foundry VTT Shattered-Sea actor Crisdalynn Khinris 2026-09-13"
  - "wiki/_archive/crissdalynn-khinriss.md"
  - "wiki/_archive/session-logs/crissdalynn-khinriss-session-log.md"
  - "wiki/_archive/combat-profile/crissdalynn-khinriss-combat-profile.md"
  - "wiki/_archive/va-scripts/crissdalynn-khinriss-voice-script.md"
summary: Level 5 Kensei Aarakocra holding the Fate Spinner at the otter camp while Talon Skarn is in her face.
provenance:
  extracted: 1.0
  inferred: 0.0
  ambiguous: 0.0
base_confidence: 0.37
lifecycle: proposed
lifecycle_changed: "2026-09-13"
tier: supporting
created: 2026-09-13
updated: 2026-09-14
type: pc
reveal: revealed
campaign: shattered-sea
visibility: dm
cssclasses: [pc-sheet]
status: alive
player: Courtney
class_levels: "Monk 5 (Way of the Kensei)"
level: 5
ac: 17
hp_max: 33
init_mod: 4
pp: 16
speed: "40 ft. walk; fly equal to walk"
foundry_id: Qkd4lEumNtyDVUxD
---
# Crissdalynn Khinriss

> [!narration] Narration
> Blue-black iridescent feathers, geometric leather armor, a chart satchel, and talons that catch the light. She moves light and irreverent until the fight starts, then the air around her gets serious.

## Identity

| Field | Value |
| --- | --- |
| Player | Courtney |
| Class / Level | Monk 5 (Way of the Kensei) |
| Species / Race | [[aarakocra\|Aarakocra]] |
| Background | Sailor (Foundry snapshot); Acolyte/cartographer also recorded [verify: `wiki/_archive/crissdalynn-khinriss.md`] |
| Age | `14` in the archived owner; D&D Beyond prints `5` [verify: `wiki/_archive/character-sheets/crissdalynn-khinriss-sheet.md`] |
| Also called | Crissdalyn |
| Home ship / base | [[Uncertainty]]; aboard at [[aruhe\|Aruhe]] |
| Training / order | [[sentinels-of-the-eyrie\|Sentinels of the Eyrie]] under [[master-kyzil\|Master Kyzil]] at [[high-eyrie\|High Eyrie]] |
| Current task | Finish the incomplete [[the-drowned-maw\|Drowned Maw]] map to end exile; she served the [[red-lady-dead-lady\|Red Lady]] for twenty-two months before it sank. |
| Faith | [[remnis\|Remnis]] is primary, with [[syranita\|Syranita]] and [[aerdrie-faenya\|Aerdrie Faenya]] at the margins; the sheet prints “Great Old Crow” [verify: `wiki/_archive/character-sheets/crissdalynn-khinriss-sheet.md`]. |
| Foundry identity | Actor `Qkd4lEumNtyDVUxD` from world Shattered-Sea, snapshot 2026-09-13; table display prefers Crissdalyn. |

## Combat Stats

| Stat | Value |
| --- | --- |
| AC | 17 (Unarmored Defense: 10 + 4 + 3); Agile Parry adds +2 AC when available. |
| HP (current / max) | `33 / 33` plus 6 temporary HP (Foundry snapshot); the archived sheet prints max `35` [verify: `wiki/_archive/character-sheets/crissdalynn-khinriss-sheet.md`]. |
| Initiative | +4 |
| Passive Perception | 16 |
| Speed | 40 ft. walk; fly equal to walk while not in medium or heavy armor. The archived sheet prints 35 ft. walk / 45 ft. fly [verify: `wiki/_archive/stats/crissdalynn-khinriss-stats.md`]. |
| Resources | Focus Points `5` per short or long rest; save DC `14`; Martial Arts die `1d8`. |
| Proficiency / Hit Die | Proficiency bonus +3; hit point die `d8`. |
| Defenses / constraints | Strength and Dexterity saves are proficient; no damage resistances are recorded. Charisma and Intelligence saves are +0, while low ceilings, wind, and nets can deny flight. |
| Pressure / synergy | Ranged focus fire and Wisdom pressure on stuns or fear are soft counters; open vertical space supports seize-and-carry, and ally advantage such as [[catarina-davirelli\|Catarina]]'s Faerie Fire helps. |


````col
```col-md
## Ability Scores

| Ability | Score | Mod | Save |
| --- | --- | --- | --- |
| Strength | 12 | +1 | +4 |
| Dexterity | 19 | +4 | +7 |
| Constitution | 12 | +1 | +1 |
| Intelligence | 11 | +0 | +0 |
| Wisdom | 16 | +3 | +3 |
| Charisma | 10 | +0 | +0 |

Strength and Dexterity are the proficient saving throws. The proficiency bonus is +3.
```
```col-md
## Skills

| Skill | Bonus | Proficiency |
| --- | --- | --- |
| Acrobatics | +7 | Proficient |
| Athletics | +4 | Proficient |
| Investigation | +3 | Proficient |
| Perception | +6 | Proficient |
| Animal Handling | +3 | Untrained |
| Arcana | +0 | Untrained |
| Deception | +0 | Untrained |
| History | +0 | Untrained |
| Insight | +3 | Untrained |
| Intimidation | +0 | Untrained |
| Medicine | +3 | Untrained |
| Nature | +0 | Untrained |
| Performance | +0 | Untrained |
| Persuasion | +0 | Untrained |
| Religion | +0 | Untrained |
| Sleight of Hand | +4 | Untrained |
| Stealth | +4 | Untrained |
| Survival | +3 | Untrained |

Passive Insight is 13 and passive Investigation is 13. Tools: Calligrapher's Supplies (Kensei) and Navigator's Tools (Sailor). Weapon proficiencies include Hand Crossbow, Scimitar, Shortsword, and Simple Weapons; no armor proficiency is listed. Languages are not recorded [unknown: `wiki/_archive/stats/crissdalynn-khinriss-stats.md`].
```
````

## Actions

### Attacks

| Attack | To-Hit / DC | Damage / Effect | Notes |
| --- | --- | --- | --- |
| [[quarterstaff\|Quarterstaff]] (Topple) | +7 | `1d8 + 4` bludgeoning | Simple, versatile Kensei weapon; equipped. |
| [[shortbow\|Shortbow]] (Vex) | +7 | `1d8 + 4` piercing | Simple, ammunition, range 80/320, two-handed Kensei weapon; equipped. |
| Unarmed Strike | +8 [verify] | `1d8 + 5` bludgeoning [verify] | Archived sheet lists a +1 gap over the Quarterstaff and Shortbow math with no explained feature; Martial Arts die is `1d8`. |
| Talons | +7 | `1d6 + Strength` slashing | Live snapshot uses `1d6` plus Strength; archived sheet records `1d8 + 4` [verify]. Aarakocra natural weapon. |

### Bonus Actions

| Bonus Action | Trigger / Cost | Effect | Uses | Recovery |
| --- | --- | --- | --- | --- |
| Martial Arts Unarmed Strike | — | Make an extra Unarmed Strike. | At will | — |
| Flurry of Blows | 1 Focus | Make two Unarmed Strikes. | 1 Focus | Short or Long Rest |
| Patient Defense | 0 or 1 Focus | Disengage, or spend 1 Focus to Disengage and Dodge. | 0 or 1 Focus | Short or Long Rest |
| Step of the Wind | 0 or 1 Focus | Dash, or spend 1 Focus to Disengage and Dash with doubled jump distance. | 0 or 1 Focus | Short or Long Rest |
| Kensei's Shot | — | A ranged Kensei-weapon attack deals an extra `1d4` damage of its weapon type until the end of the turn. | At will | — |

### Reactions

| Reaction | Trigger | Effect | Uses | Recovery |
| --- | --- | --- | --- | --- |
| Deflect Attacks | Hit by a bludgeoning, piercing, or slashing attack | Reduce damage by `1d10 + 4 + 5` (`1d10 + 9`). If reduced to 0, spend 1 Focus to redirect `2d8 + 4` damage of the same type to a creature within 5 ft. for a melee hit or 60 ft. for a ranged hit with no total cover; the creature can make a Dexterity save against `DC 14`. | 1 / reaction; redirect costs 1 Focus | Focus pool |
| Slow Fall | Falling | Reduce falling damage by 25. | 1 / reaction | — |

## Spells

### Spellcasting

| Source | Ability | Save DC | Attack Bonus | Casting focus / notes |
| --- | --- | --- | --- | --- |
| Aarakocra Wind Caller | [[wisdom\|Wisdom]] | `DC 14` | None; Gust of Wind is save-only. | Racial casting only; she has no Monk spellcasting, no cantrips, and no full prepared-caster list. |

### Prepared or Known

| Spell | Level / Pool | Notes |
| --- | --- | --- |
| [[gust-of-wind\|Gust of Wind]] | 2 / Wind Caller | Known through the racial grant; usable once per long rest. |

### Slots or Casting Resources

| Level / Pool | Current / Max | Recovery |
| --- | --- | --- |
| Wind Caller ([[gust-of-wind\|Gust of Wind]]) | 1 use | Long Rest; no material component; it can also be cast through any 2nd-level-or-higher spell slot, but that slot option is theoretical because she has no other spellcasting class. |

## Inventory

### Attuned

| Item | Notes |
| --- | --- |
| [[eldritch-claw-tattoo\|Eldritch Claw Tattoo]] | Equipped and attuned; Session 04 records a 15 ft. reach extension when active, with a once-per-day-at-dawn recharge. |
| [[fate-spinner\|Fate Spinner]] | Attuned; once-per-day vision-and-advantage effect. Kyzil gave her the crystal dreidel. |

### Carried

| Item | Qty | Notes |
| --- | --- | --- |
| [[cloak-of-displacement\|Cloak of Displacement]] | 1 | Carried, not worn, and still requires attunement. Sold as a Nightmantle; cursed so attacks aimed at her have disadvantage until she is hit, while a ranged attack aimed at a nearby ally targets her instead as a normal roll. |
| Dragonhide Belt +1 | 1 | Attunement is not confirmed; bought from [[prospero-morsani\|Prospero Morsani]] for 100 gold (Delmar paid); +1 to Focus save DCs and an action to regain `1d6` Focus once at dawn. |
| [[quarterstaff\|Quarterstaff]] (Topple) | 1 | Equipped. |
| [[shortbow\|Shortbow]] (Vex) | 1 | Equipped. |
| Backpack and quiver | 1 | Equipped. |
| Arrows | 20 | Shortbow ammunition. |
| Diamond ring | 1 | Worth 500 gold; kept from Iacopo Fieschi in Session 07. |
| [[redheart-berry]] | 5 | Fallen and plucked fruit from Session 11; one was eaten for `30` HP. |
| Rope, torches, waterskin, oil, Calligrapher's Supplies, rations, bedroll, tinderbox | — | Torches 10, oil 2, and rations 10. |

Carried weight is 68 lb. Encumbrance begins at 180 lb.; Push/Drag/Lift is 360 lb.

### Currency

| Coin | Amount |
| --- | --- |
| gp | [unknown] |

The archived sheet records no currency amounts.

## Features

### Traits

| Trait | Source | Effect |
| --- | --- | --- |
| Flight | [[aarakocra\|Aarakocra]] | Fly at walking speed; the live snapshot specifies flight equal to walk while not in medium or heavy armor. |
| Talons | [[aarakocra\|Aarakocra]] | Unarmed strike can deal `1d6 + Strength` slashing in the live snapshot; the archived sheet applies the Martial Arts die and records `1d8 + 4` [verify]. |
| Wind Caller | [[aarakocra\|Aarakocra]] | Grants [[gust-of-wind\|Gust of Wind]] once per long rest. |
| Darkvision | [[aarakocra\|Aarakocra]] | No darkvision is recorded; Jean-Claude's darkness can leave her without sight. |

### Class Features

| Feature | Source | Effect | Uses | Recovery |
| --- | --- | --- | --- | --- |
| Martial Arts | [[monk\|Monk]] | Make an Unarmed Strike as a Bonus Action; use Dexterity for Monk weapon and Unarmed Strike attacks and damage, and the `1d8` Martial Arts die for Unarmed Strikes and Monk weapons. | At will | — |
| Unarmored Defense | [[monk\|Monk]] | AC is `10 + Dexterity + Wisdom` while unarmored and without a shield, currently 17. | Passive | — |
| Monk's Focus | [[monk\|Monk]] | Focus pool fuels Flurry of Blows, Patient Defense, Step of the Wind, Stunning Strike, Read the Current, and redirected Deflect Attacks. | 5 | Short or Long Rest |
| Unarmored Movement | [[monk\|Monk]] | Adds 10 ft. speed while unarmored and without a shield. | Passive | — |
| Uncanny Metabolism | [[monk\|Monk]] | When rolling Initiative, regain all Focus and `5 + 1d8` HP. | 1 | Long Rest |
| Extra Attack | [[monk\|Monk]] | Make two attacks with the Attack action. | Passive | — |
| Kensei Weapons | [[monk-kensei\|Kensei]] | [[quarterstaff\|Quarterstaff]] and [[shortbow\|Shortbow]] are treated as Monk weapons. | Passive | — |
| Way of the Brush | [[monk-kensei\|Kensei]] | Proficiency with calligrapher's or painter's supplies. | Passive | — |
| Agile Parry | [[monk-kensei\|Kensei]] | After an Unarmed Strike as part of the Attack action while holding a Kensei weapon and not incapacitated, gain +2 AC until the start of the next turn. | At will | — |
| Stunning Strike | [[monk\|Monk]] | Once per turn after hitting with a Monk weapon or Unarmed Strike, spend 1 Focus. The target makes a Constitution save against `DC 14`; failure causes **stunned** until the start of her next turn, while success halves its Speed and gives the next attack against it Advantage. An archived DM note labels one Dragonhide Belt interaction `DC 15` [verify: `wiki/_archive/crissdalynn-khinriss.md`]. | 1 Focus | Focus pool |
| Read the Current | Kensei technique taught by [[master-kyzil\|Master Kyzil]] | Once on her turn after hitting with an Unarmed Strike or Kensei weapon, spend 1 Focus to read the target through contact; it cannot combine with Stunning Strike or Deft Strike on the same hit. Ask for one combat-observable truth: **Guard** (what defense it relies on), **Opening** (which save appears easiest to pressure), or **Pattern** (one habit such as counterattacks, reach, mobility, guarding an ally, protecting an object, concentration, brute force, or escape). No exact numbers, full stat blocks, secret identity, hidden lore, or motives; repeating on the same creature reveals a different or deeper category. | 1 Focus | Focus pool |

### Feats

| Feat | Source | Effect |
| --- | --- | --- |
| [[grappler\|Grappler]] | ASI feat | Punch and Grab: an Attack-action Unarmed Strike hit also Grapples once per turn. Gain Advantage against creatures she has Grappled. Fast Wrestler removes extra movement cost when moving a Grappled creature her size or smaller. |
| Tavern Brawler | Origin (Sailor); archived page labels it a feat [verify] | Enhanced Unarmed Strike can deal `1d4 + 1` bludgeoning instead of normal damage; reroll a 1 on an Unarmed Strike damage die and use the new roll; proficiency with improvised weapons; an Unarmed Strike hit can Push the target 5 ft. once per turn. |

## Connections

| Tie | Table consequence |
| --- | --- |
| [[delmar-fisk\|Delmar Fisk]] | He is her fixed point. She pulled him from the [[red-lady-dead-lady\|Red Lady]] crow's nest through 30 ft. of solid water, brought him back from Admiral Fisk, and later leg-swept him after he went to [[Umberlee]] without her. |
| [[master-kyzil\|Master Kyzil]] | Her mentor at [[high-eyrie\|High Eyrie]]; he gave her the Fate Spinner, acknowledged her growth in the rooftop spar, and called her pilgrimage the hardest in Eyrie history. |
| [[sentinels-of-the-eyrie\|Sentinels of the Eyrie]] | Her Kensei order; return still depends on completing the [[the-drowned-maw\|Drowned Maw]] map. |
| [[fate-spinner\|Fate Spinner]] | [[talon-vantyrus\|Talon Vantyrus]] sent a ship to take it, while Kyzil gave her the crystal dreidel that remains a live pressure. |
| [[Uncertainty]] | Home ship; she is aboard at [[aruhe\|Aruhe]]. |
| [[jean-claude-tabarnack\|Jean-Claude Tabarnack]] | Crew; his darkness leaves her without darkvision cover, so the crew must coordinate light, and he fears birds. |
| [[perrin-black-jaw\|Perrin Black-Jaw]] | Crew; she has blocked him from killing Ket and caught him during a rigging fall. |
| [[high-eyrie\|High Eyrie]] | The monastery exiled her at 14 for careless craft and goofing off. Kyzil has confirmed survival counts and delivered its weather warning, but still wants the full record and answers about two missing Sentinels; Maw knowledge may unlock the Eyrie's fissure cycle, and her return still depends on earning the monastery's forgiveness. |
| [[the-drowned-maw\|The Drowned Maw]] | Her map remains incomplete after the [[red-lady-dead-lady\|Red Lady]] sank; exile, not the wreck itself, is the unresolved wound. The Maw also links to Delmar's [[pearl-of-souls\|Pearl of Souls]] and Perrin's [[leviathan\|Leviathan]] thread. |
| [[red-lady-dead-lady\|Red Lady]] | She served aboard for twenty-two months before its sinking; Delmar is the sole survivor recorded in the archive. |

## Session Log

Kyzil's approval advanced from the S03 reunion rupture to the S04 rooftop acknowledgment. Grappling and seize-and-carry have recurred since S02, while protection of Delmar remains reciprocal.

| Session / Date | Encounter | Current-play change |
| --- | --- | --- |
| 01 | Boarding | Dragged Cap'n Gorgeous through Perrin's Minor Illusion to conceal the engagement, breached the weather deck when Delmar launched her through the hatch, and redirected Grung toxin gas with Gust. |
| 02 | Rook fight | Deflected Rook's flintlock, blocked Perrin before he killed Ket, and let Ket feed on Rook's men. |
| 03 | Open air / charts | Caught Perrin and Jean-Claude from the crow's nest; lifted a 60-ft. whip shark on a contested check after its natural 1; found Rook broke Imperial rules about the [[the-drowned-maw\|Drowned Maw]] twice and kept no record; abandoned an aerial scout of Calveno because of the crowd; reunited with [[master-kyzil\|Master Kyzil]] and broke down after he dove away. |
| 04 | Kyzil spar / sewers | Grappled Kyzil in round one after his natural 1, activated the tattoo's 15 ft. reach, deflected 13 damage and redirected it before he saved, fell from Downburst and was caught, vouched for Jean-Claude, heard that the pilgrimage was the hardest in Eyrie history, clotheslined a patrol Grung, seized the fleeing purple Grung in a 45-ft. Kingfisher dive, and leg-swept Delmar over [[Umberlee]]. |
| 06 | Primary Chamber | Deflected Tongue Lash twice, once redirecting it onto a Grung; took two Still-Water Strikes for 11 and 10 bludgeoning down to 1 HP; killed Vashu in melee; grappled and poisoned Solange before her teleport; leveled to Monk 5; and confirmed the tattoo recharges once per day at dawn. |
| 07 | Otar | Activated the tattoo for +1 to Unarmed attack and damage, landed strikes for 11 and 16, deflected an 11-damage Tongue Lash, redirected it before Otar dodged, and kept Iacopo Fieschi's diamond ring worth 500 gold. |
| 08 | Cabinet of Morsani / La Cenere | Tested the Dragonhide Belt, secured it for Delmar after the market negotiation, and bought a cloak sold as Nightmantle that was identified as a cursed [[cloak-of-displacement\|Cloak of Displacement]] from Lavinia Sordi. |
| 11 | Aruhe inland | Ate a fallen [[redheart-berry]] for `30` HP, pulled Matteo from otters, took last watch, and faced Skarn stooping for the dreidel after Stunning Strike landed and a Legendary Resistance answered. The fight remains open. |
