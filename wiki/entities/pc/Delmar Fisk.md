---
title: Delmar Fisk
category: entities
tags: [shattered-sea, pc]
sources:
  - "wiki/_archive/delmar-fisk.md"
  - "wiki/_archive/abilities/delmar-fisk-abilities.md"
  - "wiki/_archive/stats/delmar-fisk-stats.md"
  - "wiki/_archive/character-sheets/delmar-fisk-sheet.md"
  - "wiki/_archive/inventory/delmar-fisk-inventory.md"
  - "wiki/_archive/combat-profile/delmar-fisk-combat-profile.md"
  - "wiki/_archive/va-scripts/delmar-fisk-voice-script.md"
  - "wiki/_archive/interviews/delmar-fisk-interview.md"
  - "Session 01 - Recap.md"
  - "Session 04 - Recap.md"
  - "Session 07 - Recap.md"
  - "Session 10 - Recap.md"
  - "Foundry VTT Shattered-Sea actor Admiral Delmar A. Fisk 2026-09-13"
summary: Level 5 Swashbuckler at 35/38 HP who stole the Pearl of Souls and now carries Umberlee's price.
provenance:
  extracted: 0.9
  inferred: 0.1
  ambiguous: 0.0
base_confidence: 0.42
lifecycle: canon
lifecycle_changed: "2026-09-12"
tier: supporting
created: 2026-09-12T04:56:01Z
updated: 2026-09-13T23:40:00Z
type: pc
reveal: revealed
campaign: shattered-sea
visibility: dm
status: alive
role: PC
location: Uncertainty
player: Frederick
foundry_id: O68NGfrVI6ibOW9v
relationships:
  - target: "[[Pearl of Souls]]"
    type: related_to
  - target: "[[Uncertainty]]"
    type: related_to
  - target: "[[Flying Boots]]"
    type: related_to
---
# Delmar Fisk

Party member. Frederick plays him. The table has heard him as both Delmar and Admiral Fisk. Foundry actor name: Admiral Delmar A. Fisk.

## At a Glance

| **Role**          | PC |
| ----------------- | -- |
| **Class / Level** | Rogue 5 (Swashbuckler) |
| **Player**        | Frederick |
| **Home ship**     | [[Uncertainty]] |

> **DM thesis:** Charm first, then Admiral Fisk when the pressure hits. Sneak Attack rides the blunderbuss. [[Crissdalynn Khinriss]] has already pulled him back once.

## Connections

- [[Pearl of Souls]] — He stole it. [[Umberlee]] named it as her price after reading his offer out of his head.
- [[Uncertainty]] — Berth and station.
- [[Barnaby Rook]] — Delmar put him in the water.
- [[Geoffrey Draves]] — Asked which side would keep him alive. Now training him on night watches.
- [[Catarina Da'Virelli]] — Gave him Lamarae's Fang. Enchanted the blunderbuss mid-fight.
- [[Vashka Doru]] — Has a harbor secret about a ship that once belonged to one of the five souls he carries.

Umberlee named the Pearl as her price after reading Delmar's offer out of his head. Catarina Da'Virelli gave him Lamarae's Fang. He is aboard *[[Uncertainty]]* at [[Aruhe - Hungry Isle]]. See [[journal/Session 04 - Recap]] and [[Story So Far]].

## Foundry sheet

Live snapshot from Foundry world Shattered-Sea on 2026-09-13. Actor `O68NGfrVI6ibOW9v`. Variant Human Rogue 5, Swashbuckler (SCAG), Sailor. Proficiency Bonus `+3`. Hit Point Die `d8`. Current HP **35/38**.

| Field | Value |
| --- | --- |
| **AC** | `16` (leather) |
| **Speed** | Walk `30` ft. [[Flying Boots]] spend a charge for Fly `30` ft. for 1 hour. Cloak of the Manta Ray grants Swim `60` ft. and water breathing while worn. |
| **Initiative** | Dexterity `+5` plus Charisma `+2` from Rakish Audacity |

| Ability | Score | Mod | Save |
| --- | --- | --- | --- |
| Strength | 10 | `+0` | `+0` |
| Dexterity | 20 | `+5` | `+8` |
| Constitution | 14 | `+2` | `+2` |
| Intelligence | 11 | `+0` | `+3` |
| Wisdom | 12 | `+1` | `+1` |
| Charisma | 15 | `+2` | `+2` |

Saves: Dexterity and Intelligence.

| Skill | Bonus | Training |
| --- | --- | --- |
| Acrobatics | `+8` | proficient |
| Insight | `+4` | proficient |
| Investigation | `+6` | expertise |
| Perception | `+4` | proficient |
| Persuasion | `+8` | expertise |
| Sleight of Hand | `+8` | proficient |
| Stealth | `+8` | proficient |

Untrained: Animal Handling `+1`, Arcana `+0`, Athletics `+0`, Deception `+2`, History `+0`, Intimidation `+2`, Medicine `+1`, Nature `+0`, Performance `+2`, Religion `+0`, Survival `+1`.

Tools: Thieves' Tools, Tinker's Tools, Navigator's Tools from Sailor. Languages include Thieves' Cant.

### Features in play

- **Sneak Attack.** Once per turn, extra `3d6` with a Finesse or Ranged hit that has Advantage, or when an ally is within `5` ft. of the target and Delmar does not have Disadvantage. Rakish Audacity also lets him Sneak Attack a creature within `5` ft. if no other creatures are within `5` ft. of him.
- **Cunning Action.** Bonus Action Dash, Disengage, or Hide.
- **Steady Aim.** Bonus Action Advantage on the next attack this turn if he has not moved; Speed becomes `0` until the end of the turn.
- **Cunning Strike.** When Sneak Attack damage lands, forgo `1d6` for Poison (**Constitution save — `DC 16`**, needs a Poisoner's Kit), Trip (**Dexterity save — `DC 16`**, Large or smaller → **prone**), or Withdraw (move up to half Speed with no Opportunity Attacks).
- **Uncanny Dodge.** Reaction to halve damage from an attack he can see.
- **Fancy Footwork.** A melee attack on his turn stops that target's Opportunity Attacks against him for the rest of the turn.
- **Firearm Specialist.** Firearm proficiency. On a misfire, Reaction `d20`; higher than the misfire score prevents the misfire.
- **Lucky.** Luck Points equal to Proficiency Bonus (`3`); spend on a D20 Test; regain on a Long Rest.
- **Tavern Brawler.** Unarmed Strike can deal `1d4` plus Strength bludgeoning; reroll 1s on those damage dice.

### Equipped and carried

Attuned: Cloak of the Manta Ray, [[Flying Boots|Winged Boots]] (`4` charges, regain `1d4` at dawn).

| Item | State |
| --- | --- |
| Leather armor | equipped |
| Rapier (Vex) | equipped |
| 2 daggers (Nick) | equipped |
| 3 pistols (Vex) | equipped |
| Unknown thing. (pistol, Vex) | equipped; Foundry name is literal |
| Blunderbuss (Exandria) | equipped |
| Unarmed Strike | equipped |
| Bag of Holding | equipped |
| Backpack | equipped |
| Musket bullets | `40` |
| Blunderbuss bullets | `5` |
| Potion of Healing (Greater) | `4d4 + 4` HP |
| The Fleet Commander's Chair | carried |
| Common clothes, iron pot, shovel, Tinker's Tools, Thieves' Tools | carried |
| Oil `7`, ball bearings `1000`, rations `5`, rope, bell, tinderbox, waterskin, hooded lantern, candles `10`, crowbar | carried |

## Connections

| Relationship | Meaning |
| --- | --- |
| [[Crissdalynn Khinriss]] | She has already pulled him out of Admiral Fisk once. |
| [[Uncertainty]] | He is aboard at [[Aruhe - Hungry Isle]]. |
| [[Pearl of Souls]] | Umberlee's outstanding price for his life. |
| [[Flying Boots]] | Attuned on his feet; provenance still contested. |
| [[Geoffrey Draves]] | Night-watch trainee. |
