---
title: "Uncertainty - Manifest"
category: entities
tags: [shattered-sea, vehicle]
sources:
  - "campaign-os:uncertainty-manifest.md"
created: 2026-09-13
updated: 2026-09-13
type: vehicle
lifecycle: proposed
lifecycle_changed: "2026-09-13"
reveal: unrevealed
campaign: shattered-sea
visibility: dm
summary: "Everything aboard the Uncertainty space by space: ordnance and powder, stores, medical, charts, the expanded cargo hold, and the cargo still worth selling."
provenance:
  extracted: 0.85
  inferred: 0.10
  ambiguous: 0.05
base_confidence: 0.55
tier: supporting
---
# Uncertainty - Manifest

> [!narration] Narration
> Down past the companionway the air changes — tallow smoke, bilge salt, cold iron off the boarding axes stacked in an open crate. Lantern light swings with the hull and catches crate after crate lashed along the bulkheads: sealed powder kegs chalked with tally marks, water casks, the folding cots pushed under a bunk since the surgeon's alcove became a workbench. The seized-goods crate stands cracked open again, oilcloth peeled back from a jewellery box nobody's forced and a pendant nobody's sold — *For Mira, from the sea*. One crate further aft stays closed, its lid marked only with a chart of the Galewall anchorage nobody has agreed to open.

*Patrol cutter · Tier 1 · [[La Vasca]].*

What she carries, and where. Spaces and their tactical use live on [[Uncertainty - Deck Layouts|Deck Layouts]]. This page holds the stock in them. Every Crown-issue item went ashore, into a sale, or into a replacement.

## Stats & Combat

Hull numbers are on [[Uncertainty|the ship's page]]. This is the ordnance and everything that feeds it.

### Weather Deck

| Item | Qty | Notes |
|---|---|---|
| Long 9-pounder on truck carriage | 2 | Forward mount each side. The only guns that bear at Cannon range |
| Short 32-pounder on truck carriage | 2 | Aft mount each side. Boarding range only, and it hits like the number says |
| Swivel gun | 2 | Bow on a full pintle, stern on a 180° aft arc |
| Ram plate | 1 | Iron-capped, rebedded flush at the waterline |
| Figurehead | 1 | [[Le Paludi]] carving, unnamed |
| Signal lantern | 1 | Aft rail, paired with the one below. Twenty-five words to thirty miles |
| Capstan, anchor hawse, spare line | n/a | Fixed |
| Jolly boat | 1 | Ship's tender |

### Powder Magazine

| Item | Qty | Notes |
|---|---|---|
| 9-pounder charge, sealed copper | 24 | Delivered with the guns |
| 32-pounder charge, sealed copper | 16 | Heavy. The magazine fills at this count |
| Swivel and pistol charge | 20 | Flintlock loads |

### Expanded Cargo Hold

| Item | Qty | Notes |
|---|---|---|
| Cargo netting | floor to ceiling | Held taut against the roll |
| Racked crates, empty | 6 | Ready for whatever the next job pays in |
| Block and tackle | 1 | For anything too heavy to carry up from the hold |

[[Delmar Fisk|Delmar]] keeps a loaded blunderbuss of his own close at hand, one shot of 4d6 in a 15-ft cone before it wants reloading.

## Crew

- Captain: [[Delmar Fisk]].
- Notable crew: [[Cobb]], [[Sem Holst]], [[Jean-Claude Tabarnack]] — each keeps something aboard outside the common stores, listed below.

Not everything aboard belongs to the ship. [[Sem Holst]]'s carpenter's kit is his own and travels with him. [[Cobb]] holds the seal on the emergency ration chest, which opens at [[Nona Black-Jaw]]'s word and nobody else's. [[Jean-Claude Tabarnack|Jean-Claude]] keeps the [[Grung]] poison tincture under his own hand. The rest is common stores, and the roster that spends them is on [[Uncertainty|the ship's page]].

### Provisions Store

| Item | Qty | Notes |
|---|---|---|
| Salt fish, rice, dried beans, cooking oil | n/a | Thirty days for a full complement of ten |
| Fresh citrus | n/a | Ten days before it turns |
| Water cask | 4 | |
| Small beer barrel | 2 | |
| Emergency ration chest, sealed | 1 | Seven days for ten. Cobb's seal |
| [[Mending]] Resin | 2 | Carpenter's locker. Restores 2d8 hull over eight hours, no roll |

### Galley

| Item | Qty | Notes |
|---|---|---|
| Cast-iron stove on sand | 1 | Lit, fixed, in the hold below the mess |
| Cook's tools, bread locker | n/a | Unused. No cook aboard |

### Rigger's Workshop

| Item | Qty | Notes |
|---|---|---|
| Workbench, vise | 1 | Bolted, forward alcove off the mess |
| Tinker's tools | 1 set | [[Catarina Da'Virelli|Catarina]] outfitted this rack |
| Carpenter's tools | 1 set | Backs up Sem Holst's own kit |
| Jeweler's tools | 1 set | |
| Glassblower's tools | 1 set | |
| Leatherworker's tools | 1 set | |
| Woodcarver's tools | 1 set | |

### Chart Archive

| Item | Qty | Notes |
|---|---|---|
| Survey charts, [[Crown Islands]] and [[Central Strait]] | n/a | Admiralty marks scraped, [[Calven and Calveno]] notation laid over. Good coverage, no provenance |
| Navigation dividers | 1 set | |
| [[Barnaby Rook|Rook]]'s captain's log | 1 | Still worth reading |
| [[Letters of Marque]] | 1 set | Rook's privateer authority. A forger could do a great deal with them |

### Crew Quarters and Mess

| Item | Qty | Notes |
|---|---|---|
| Hammock hooks and kit lockers | 6 | Delmar's among them |
| Long table, bulkhead bench | 1 | Cut in aft where the captain's cabin was |
| Armchair | 1 | Cobb's find |
| Cards and dominoes | 1 set | Turned up in a tool chest and stayed, without comment |
| Folding cot | 2 | Carried out of the old surgeon's alcove when it became the workshop |
| Suture kit, splints, fever treatments | n/a | Carried out with the cots, stowed under a bunk now |
| [[Potion of Healing]] | 3 | No chandlery on the Chain stocks these |

### General Cargo

| Item | Qty | Notes |
|---|---|---|
| Seized goods crate | 1 | About 97 gp of silver candelabras, brandy, [[Calven and Calveno]] silk, a locked jewellery box, and a pendant cut *For Mira, from the sea* |
| Confiscation crate | 1 | Spare flintlocks, and twenty vials of concentrated [[Grung]] poison tincture |
| Opened cargo crate | 1 | Emptied. Held a chart marked *[[Galewall]] anchorage* |
| Boarding axe | 8 | Replaced the Crown cutlasses. No rack anymore — dug out of a crate now |
| Crossbow | 4 | Twenty bolts each |
| Grapnel line | 6 | |
| Smoke grenade | 3 | |
| Incendiary flask | 2 | |
| Arms chest, lockable | 1 | The old weapons-locker chest, moved down with everything else |
| Caulking hammer set | 1 | [[Cobb]]'s, and useful as a weapon, he notes |
| Spare cutlass | 1 | Off the old cabin rack, now on the mess bulkhead |

Nobody aboard has worked out what waits at the Galewall anchorage, and the chart stays in the crew's hands.

## Connections

- [[Uncertainty]]. Hull, armament, crew, history.
- [[Uncertainty - Deck Layouts|Deck Layouts]]. The spaces this stock sits in.
- [[Ship Operations]]. Weekly facility orders that spend and replenish it.
- [[Cobb]]. Stocked the refit, and holds the ration seal.
- [[Nona Black-Jaw]]. Paid for it, and decides when the chest opens.

## Session Log

- [[Session 2]]. The crew stripped the cabin and hold of coin, garnets, the blunderbuss, and Rook's letters.
- Session 8. The batteries, powder, and full stores landed aboard in the cradle.
