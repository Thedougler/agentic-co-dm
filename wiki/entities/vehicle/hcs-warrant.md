---
title: "HCS Warrant"
aliases:
  - HCS Warrant
category: entities
tags: [shattered-sea, vehicle]
sources:
  - "campaign-os:hcs-warrant.md"
created: 2026-09-13
updated: 2026-09-13
type: vehicle
lifecycle: proposed
lifecycle_changed: "2026-09-13"
reveal: unrevealed
campaign: shattered-sea
visibility: dm
summary: "Crown's primary enforcement brigantine on the Central Strait, crewed from Harwick"
provenance:
  extracted: 0.85
  inferred: 0.10
  ambiguous: 0.05
base_confidence: 0.55
tier: supporting
---
# HCS Warrant

> [!narration] Narration
> A Crown brigantine bears down under full sail. Naval grey hull with a white stripe. Gun ports line each side: six per beam, twelve total. The brass plate at the bow is large enough to read. A rating officer stands at the rail with a speaking trumpet. The Crown pennant hangs still at the masthead. She comes downwind and close. She makes you feel it. She doesn't ask.

The *HCS Warrant* patrols the [[central-strait]] with crews from [[Harwick]]. She works the seas near [[calders-tooth-and-port-tidefall]], larger and better armed than Tidefall's patrol cutters. She makes ships comply where cutters alone cannot. She enforces Crown authority.

[[commander-gideon-ault]] commands. Sealed orders grant him power to detain ships, seize cargo, arrest crews, and issue privateer letters to allies.

**Enforcement posture:** the *Warrant* comes downwind and close. She is ready to talk, but the message is clear. A ship that sees twelve gun ports has already decided.

**Privateer letters:** Ault issues them. The [[Harwick]] ledger records them at resupply.

**Prize posture:** capture of the *Warrant* reaches the Admiralty at [[Harwick]] within a week. Sealed orders go to every Crown ship in the Strait.

## Layout

Deck notes from the *Warrant*:

- **Weather Deck.** Helm and binnacle aft. Belfry and capstan amidships. Swivel guns cover bow and stern. Cannon (twelve total) below with hinged covers. Crown pennant flies from the masthead.
- **Captain's Cabin and Officers.** Stern cabin (full width) with chart table and papers. Lockable iron locker inside. Officer cabins (three) forward. Rating officer's desk amidships for paperwork.
- **Crew Quarters.** Hammocks (twenty) on the lower forward deck. [[Galley]] with copper range amidships.
- **Below the Waterline.** Magazine aft (two keys: Ault and [[Harwick]] Admiralty). Cargo hold for seized goods awaiting judgment. Prisoner hold with iron grating and six cages.

> [!mechanic]
> **Captain's Locker.** Lockable iron chest in the stern cabin. Make a Strength (Athletics) check (DC 18) to force it. If you fail, the lock holds and the chest remains undamaged. You may retry only with a new tool or approach.

## Stats & Combat

| | |
|---|---|
| **Tier** | 2 |
| **Variant** | Armed |
| **Decks** | 3 |
| **Hull Points** | 210 |
| **Hull AC** | 12 |
| **[[Condition]]** | Pristine |
| **[[Speed]] (good wind)** | 80 miles/day |
| **Speed (poor wind)** | 40 miles/day |
| **Maneuverability** | Average |
| **Profile** | High (visible at distance) |
| **Crew (min/full)** | 10 / 24 |
| **Cargo** | 36 tons (48 total capacity) |
| **Gun Mounts** | 12 (6 per side) |
| **Weapons** | 10 Cannon, 2 Swivel Guns |
| **Upkeep** | ~115 gp/week |
| **Space** | 8 units (Tier 2 cap) |

**Facilities** (Bastion mapping):

| Feature | Bastion Type | Space |
|---|---|---|
| Magazine and arms locker | Weapons Locker | 1 |
| Officer charts and navigation | Chart Archive | 2 |
| Captain's cabin | Helm and [[Command]] | 1 |
| Crew berths and galley | Crew Berths | 1 |

Installed: 5 units. Open: 3 units. Special slot: 1 available.

## Crew

Captain [[commander-gideon-ault]]. Twenty-four sailors from [[Harwick]]. Watch officers (three), gunner, carpenter. Inspectors (two per action) board to check cargo and verify papers.

## Connections

- [[commander-gideon-ault]]: commands
- [[dravosi-crown]]: registered authority
- [[Uncertainty|HCS Surety]]: the missing vessel driving this deployment
- [[hcs-relentless]]: fleet sibling
- [[central-strait]]: patrol waters
- [[calders-tooth-and-port-tidefall]]: patrol approaches

## Sister Ships

*[[Vethka]]* serves a different faction and fleet. A [[Grung]] proa crewed by Grung, not Crown sailors. The real sister ships are the Crown's HCS vessels: [[Uncertainty|HCS Surety]] and [[hcs-relentless]].
