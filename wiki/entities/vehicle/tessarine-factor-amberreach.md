---
title: "Tessarine Factor Amberreach"
aliases:
  - Tessarine Factor Amberreach
category: entities
tags: [shattered-sea, vehicle]
sources:
  - "campaign-os:tessarine-amberreach.md"
created: 2026-09-13
updated: 2026-09-13
type: vehicle
reveal: unrevealed
campaign: shattered-sea
visibility: dm
summary: "A Tier 3 Tessarine cargo galleon serving the commercial route from Calveno westward."
provenance:
  extracted: 0.85
  inferred: 0.10
  ambiguous: 0.05
tier: supporting
---
# Tessarine Factor Amberreach

> [!narration] Narration
> The ship sits low and wide. Her grey hull is heavy with cargo. She carries freight, not speed. The beam spans wide. Cargo ports line her sides with neat numbers. A Tessarine flag flies from the mast. Gun ports sit open on her rail. No one approaches them without worry.

A Tier 3 Tessarine cargo galleon. She sails from [[calven-and-calveno]] west through the [[central-strait]]. The Concordat files all trips three ports ahead. She insures cargo and pays most losses within forty days. Merchants rely on her.

The *Amberreach* gets what she wants through reliability. Her gun ports sit open as a warning to pirates. She does not attack merchants. They board her of their own will and walk away alive.

Senior Factor [[oriana-becht]] signs legal papers aboard: loans to 2,000 gp, route deals, salvage plans, and cargo oaths. Her cabin (Captain's Locker) sits in the stern officer suite, near the contract office and secure mail.

> [!mechanic]
> **Captain's Locker.** Senior Factor Becht's locked cabin cabinet. A DC 18 Strength check breaks it open. Failure brings factor suite staff to the spot.

**The bonded compartment** holds cargo that the Crown will not inspect under a trade treaty. Both the Concordat and Dravosi Admiralty hate this rule but follow it anyway. One sealed hold stores Tessarine goods with wax seals, and the forward compartment is the bond store.

## Stats & Combat

| | |
|---|---|
| **Tier** | 3 |
| **Hull Points** | 340 |
| **Hull AC** | 13 |
| **Speed (good wind)** | 65 miles/day |
| **Speed (poor wind)** | 30 miles/day |
| **Crew (min/full)** | 22 / 60 |
| **Cargo** | 217 tons rated (185 available) |
| **Gun Mounts** | 16 (8 per side) |
| **Weapons** | 14 x Cannon; 2 x Swivel Gun |
| **Upkeep** | ~350 gp/week |

**Weather Deck.** Cargo logistics focus: davits, winches, cargo booms. Tessarine pennant and factor's authority pennant both fly.

**Gun Deck / Factor Suite.** Sixteen cannon are widely spaced for cargo movement. The stern factor suite has Becht's cabin, a contract office, and sealed correspondence.

**Crew/Passenger Deck.** 60 hammocks forward, a galley, and six private merchant passenger cabins aft.

**Hold.** The hold contains four sealed cargo compartments with numbered sealed manifests. One holds Tessarine bonded cargo with legally binding wax seals. The forward compartment is the sealed bond store.

| Feature | Mapping | Space |
|---|---|---|
| Factor's contract office and archive | Chart Archive | 2 |
| Navigator's chart room | Navigator's Chart Room | 1 |
| Senior factor's private cabin | Captain's Cabin | 1 |
| Sealed cargo and bonded hold | Expanded Cargo Hold | 2 |
| Crew hammocks | Crew Berths | 1 |
| Working galley | Ship's Galley | 1 |
| Surgeon's station | Surgeon's Berth | 1 |
| Merchant passenger cabins | Officer's Mess | 1 |
| Carpenter's workshop | [[Carpenter's Shop]] | 1 |
| Route underwriting room | Trading Post (Special) | 2 |
| [Open] | unallocated | 2 |

## Crew

- **Captain** (Senior Factor): [[oriana-becht]]
- Notable crew: none named beyond the senior factor in the source.

## Connections

- [[tessarine-concordat]], owner
- [[oriana-becht]], senior factor
- [[calven-and-calveno]], home port
- [[calders-tooth-and-port-tidefall]], inspection stop
- [[Tessarine Silkvane]], Concordat dispatch sloop
