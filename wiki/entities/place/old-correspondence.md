---
title: "Old Correspondence"
aliases:
  - "Old Correspondence"
category: entities
tags: [shattered-sea, place]
sources:
  - "campaign-os:locations/midchain/old-correspondence.md"
summary: "An isolated islet where a lone scribe has spent decades drafting one unfinished letter to an unnamed recipient."
provenance:
  extracted: 1.0
  inferred: 0.0
  ambiguous: 0.0
tier: supporting
created: 2026-09-14
updated: 2026-09-14
type: place
reveal: unrevealed
campaign: shattered-sea
visibility: dm
kind: island
region: "Northern Midchain"
relationships:
  - target: "[[midchain-north]]"
    type: related_to
  - target: "[[Midchain]]"
    type: related_to
---
# Old Correspondence

## Overview

> [!narration] Narration
> A single stone cottage sits alone on the islet, a lamp burning in its one window day and night.
> 
> Inside, an old man writes in careful, unhurried script, an enormous stack of crossed-out drafts beside him.
> 
> *The scribe*: *(dry)* Almost done.
> 
> He said the same thing last decade.
Old Correspondence's sole resident has been drafting the same letter for as long as anyone can remember, addressed to a name nobody's ever managed to place.

## At a Glance

| Field | Detail |
|---|---|
| Type | Single-resident islet |
| Region | [[midchain-north\|The Northern Midchain]] |
| Controlled By | One resident scribe |
| Known For | A decades-unfinished letter to an unknown recipient |

## Where

- North: [[munai|Munai]]
- East: [[nemwe|Nemwe]]
- South: [[yssenmoor|Yssenmoor]]
- West: [[drovannis|Drovannis]]

## What

- Salt-bleached grass and wind-bent scrub: the wind and salt spray keep the islet's thin soil cropped low, and the cottage window looks over the whole islet.
- Sea purslane: mats over the tideline rock in fleshy, salty clusters, edible raw and enough to cut a day's rations for a party that knows to look for it.
- Gulls and crabs: gulls nest along the seaward ledges and crabs work the tide pools below, the ordinary noise and traffic of the shore.
- A swarm of rats: nests under the floorboards among the discarded drafts, and comes up biting if anyone starts pulling papers out of the stack.
- Vultures: circle the cottage roof some mornings and stoop at anything that goes still too long on the open rock outside.
