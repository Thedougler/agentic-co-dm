---
title: "Ghal Idrissa"
aliases:
  - "Ghal Idrissa"
category: entities
tags: [shattered-sea, place]
sources:
  - "campaign-os:locations/midchain/ghal-idrissa.md"
summary: "A silent Dravosi Crown quarantine stockade holding three empty plague ships, none of whose crews have ever left."
provenance:
  extracted: 1.0
  inferred: 0.0
  ambiguous: 0.0
base_confidence: 0.55
lifecycle: proposed
lifecycle_changed: "2026-09-14"
tier: supporting
created: 2026-09-14
updated: 2026-09-14
type: place
reveal: unrevealed
campaign: shattered-sea
visibility: dm
kind: island
region: "Northern Midchain"
relationships:
  - target: "[[midchain-north]]"
    type: related_to
  - target: "[[Midchain]]"
    type: related_to
---
# Ghal Idrissa

## Overview

> [!narration] Narration
> A rotting quarantine flag hangs limp over a stockade of grey driftwood, and the harbour beyond it holds three ships riding empty.
> 
> No smoke rises from any of them, and nobody waves you off. The silence does that instead.
> 
> A bell mounted at the stockade gate hangs still, its rope frayed through.
Ghal Idrissa is where the [[dravosi-crown|Dravosi Crown]] sends ships suspected of carrying plague, and where those ships stay. The quarantine has no formal end date, only a flag nobody has taken down.

## At a Glance

| Field | Detail |
|---|---|
| Type | Quarantine island |
| Region | [[midchain-north|The Northern Midchain]] |
| Controlled By | [[dravosi-crown|Dravosi Crown]] (quarantine authority) |
| Known For | Ships sent here are not expected to leave |

## Where

- North: [[central-strait|The Central Strait]]
- East: [[the-long-wager|The Long Wager]]
- South: [[sparhold|Sparhold]]
- West: [[quillane|Quillane]]

## What

- Salt scrub and driftwood grass: low, wind-flattened cover that never rises past the knee, so nothing on this island hides a person standing upright.
- Bitter samphire: grows thick along the tideline and strips off the rock in fistfuls, edible raw for anyone willing to eat something that grew this close to the quarantine flag.
- [[crab|Shore crabs]]: work the hulls of the three anchored ships at low tide, and pull free of anything soft enough to give.
- [[giant-vulture|Giant vultures]]: hold a slow circuit over the stockade from first light, and never once land on the ships themselves.
- [[swarm-of-rats|Rat swarms]]: move between the ships' bilges and the stockade's grain store on the ebb tide, the one crossing nobody on the island has managed to close off.
- [[stirge|Stirges]]: nest in the driftwood palisade after dark and drop onto anything that stops moving near the gate.

## Loot

![[ghal-idrissa-quarantine-cache]]
