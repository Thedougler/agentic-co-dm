---
title: "Whit's Larder"
aliases:
  - "Whit's Larder"
category: entities
tags: [shattered-sea, place]
sources: ["midchain-west.md"]
summary: "A western Midchain site associated with impossible cuisine made from ingredients that should not be local."
provenance:
  extracted: 1.0
  inferred: 0.0
  ambiguous: 0.0
tier: supporting
created: 2026-09-13T21:30:00Z
updated: 2026-09-20
type: place
reveal: unrevealed
campaign: shattered-sea
visibility: dm
kind: site
region: Midchain
relationships:
  - target: "[[Midchain]]"
    type: related_to
  - target: "[[Kalowe]]"
    type: related_to
---
# Whit's Larder

## Overview

> [!narration] Narration
> Whit's Larder sits among the close-set islands and shallow green channels of the western [[Midchain]]. The approach offers no ordinary market or familiar shoreline to name from the record: this place is known for impossible cuisine, made from ingredients that should not be local at all. Beyond the surrounding reef water, the exact building, cook, and landing are not established.

---

## At a Glance

[[whits-larder]] is a western Midchain site associated with cuisine made from ingredients that should not be local. The western approaches are busy, shallow, and served by negotiable local pilots; [[Kalowe]] is the recorded related harbor. No cook, kitchen, menu, ingredient, settlement, or route distance is established. Skipping the site avoids its food and whatever information its impossible ingredients could provide.

---

## If the party

- **Approaches by water:** The party enters the western Midchain's close-set island channels, where shifting sandbars and shallow water make local knowledge useful. The exact landing at Whit's Larder is not recorded.
- **Seeks the larder:** The party can look for the place associated with impossible cuisine. The cook, entrance, price, and request are Unknown.
- **Tastes or examines the food:** The source establishes ingredients that should not be local, but names no dish or effect. Keep the result open to play rather than assigning a mechanic here.
- **Leaves for Kalowe:** [[Kalowe]] is a recorded related harbor, but this page does not establish a route or travel time.

## Who

No cook, host, supplier, resident, or customer is named. The site is associated with cuisine, but the source does not establish who prepares it or whether anyone currently occupies the place.

## What

- **Impossible cuisine:** Food made from ingredients that should not be local is the site's defining feature.
- **Unlocal ingredients:** Their identity, origin, preservation, and route to the western Midchain are not recorded.
- **Western approach water:** Close-set islands, shallow channels, shifting sandbars, and local pilots shape access to the site.

## Where

- **North:** Whit's Larder lies in the western [[Midchain]]. A named northern neighbor and travel distance are not recorded.
- **East:** [[Kalowe]] is a recorded related harbor, but the direction, route, and distance are not established here.
- **South:** No named southern neighbor or route is recorded.
- **West:** The western Midchain approaches [[Galewall]], but no direct route or travel distance from Whit's Larder is established.
- **Local access:** The region's channels are shallow and sandbars shift with the season; a pilot's use and the site's exact landing remain Unknown.

## Why

Whit's Larder matters because it turns food into evidence: the ingredients themselves contradict what the western Midchain should be able to supply. It gives the party a reason to investigate a place, person, or supply chain without prescribing what the impossible ingredients mean. The page establishes no owner, recipe, origin, or consequence beyond that contradiction.