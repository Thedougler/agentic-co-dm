---
title: "Ussamet"
aliases:
  - Ussamet
category: entities
tags: [shattered-sea, place]
sources: ["midchain-west.md"]
summary: "Western Midchain ruins that surface only at the lowest tides, on a schedule the sea alone appears to keep."
provenance:
  extracted: 1.0
  inferred: 0.0
  ambiguous: 0.0
tier: supporting
created: 2026-09-13T21:30:00Z
updated: 2026-09-20
type: place
reveal: unrevealed
campaign: shattered-sea
visibility: dm
kind: site
region: Midchain
relationships:
  - target: "[[Midchain]]"
    type: related_to
  - target: "[[kalowe]]"
    type: related_to
---
# Ussamet

## Overview

> [!narration] Narration
> Ussamet is a western [[Midchain]] site of half-sunken ruins. Most of the structure lies below the water, and only the lowest tides expose enough of it to cross or inspect. The timing is a sea-kept schedule rather than a reliable appointment; the ruins' shape, age, and surviving contents are not recorded.

---

## At a Glance

[[ussamet]] is a western Midchain ruin that surfaces only at the lowest tides. The sea appears to keep its own schedule. [[kalowe]] is a recorded related harbor, but no route or distance is established. Skipping Ussamet avoids a tide-dependent ruin approach, but also gives up the short window when its exposed sections can be examined.

---

## If the party

- **Waits for the lowest tide:** The party watches the water withdraw until the ruins become accessible enough to inspect. The tide timing and duration are not recorded.
- **Crosses the exposed ruins:** The party moves through half-sunken structure whose footing, entrances, and stable sections are Unknown.
- **Searches the ruins:** The party can inspect the exposed portions for history, objects, or routes. No specific find or clue is established.
- **Stays after the water returns:** The party risks losing the exposed route as the tide rises. The rate, safe retreat, and consequences are not recorded.
- **Leaves for Kalowe:** [[kalowe]] is a recorded related harbor, but this page does not establish a route or travel time.

## Who

No resident, keeper, salvager, or guardian is established at Ussamet. The site is visited only as far as the recorded canon establishes a lowest-tide window; who else watches that window is Unknown.

## What

- **Half-sunken ruins:** The site's main feature, with most of the structure below water.
- **Lowest-tide exposure:** The ruins surface only at the lowest tides, creating a temporary access window.
- **Western channels:** Close-set islands, shallow water, shifting sandbars, and pilot routes shape the wider approach.

## Where

- **North:** Ussamet lies in the western [[Midchain]]. A named northern neighbor and travel distance are not recorded.
- **East:** [[kalowe]] is a recorded related harbor, but the direction, route, and distance are not established here.
- **South:** No named southern neighbor or route is recorded.
- **West:** The western Midchain approaches [[galewall]], but no direct route or travel distance from Ussamet is established.
- **Local access:** The ruins are accessible only during the lowest tide; the landing point and approach are Unknown.

## Why

Ussamet matters because access itself is temporary and observable: the party must recognize the lowest tide, enter the half-sunken ruins, and retreat before the sea closes the window. The ruin's origin, contents, and schedule remain open until established in play.