---
title: "Adwe"
aliases:
  - "Adwe"
category: entities
tags: [shattered-sea, place]
sources:
  - "campaign-os:locations/midchain/adwe.md"
summary: "Mangrove channels that fork three ways with no marker on any of them and rearrange after every storm, where the only pilot worth having is a heron."
provenance:
  extracted: 1.0
  inferred: 0.0
  ambiguous: 0.0
base_confidence: 0.55
lifecycle: proposed
lifecycle_changed: "2026-09-14"
tier: supporting
created: 2026-09-14
updated: 2026-09-14
type: place
reveal: unrevealed
campaign: shattered-sea
visibility: dm
kind: island
region: "Northern Midchain"
relationships:
  - target: "[[midchain-north]]"
    type: related_to
  - target: "[[Midchain]]"
    type: related_to
---
# Adwe

## Overview

> [!narration] Narration
> Mangrove roots close overhead in a tangled arch, and the channel ahead forks three ways with no marker at any of them.
> 
> Mud sucks and pops beneath the hull, and the air reeks of rot, while a heron stands motionless at the second fork, watching the boat instead of the water.
> 
Adwe's mangrove channels rearrange themselves after every storm strong enough to shift the mud, so no chart of the island stays accurate for long. Locals swear the herons never lose the way, even when the water does.

## At a Glance

| Field | Detail |
|---|---|
| Type | Mangrove maze island |
| Region | [[midchain-north|The Northern Midchain]] |
| Controlled By | Ungoverned |
| Known For | Channels that don't match the last chart after a storm |

## Where

- North: [[central-strait|The Central Strait]]
- East: [[barrow-ait|Barrow Ait]]
- South: [[corrigans-rest|Corrigan's Rest]]
- West: [[midchain-west|The Western Midchain]]

## What

- Red mangrove roots: arch overhead and close out most of the sky. The same roots run below the waterline, and they force a boat to hold the marked channel instead of cutting a shorter line.
- Nipa palms and mangrove apple: cluster on the drier hummocks between channels. The apple's pulp is bitter raw, but boiled down it makes a preserve that keeps through a long crossing.
- Mud crabs: pack the exposed roots at low tide, thick enough to fill a sack within the hour. Half the shells here run soft and split underfoot.
- Herons: stand motionless at the channel forks, reading water the mud won't show. A party that loses the one it's following loses Adwe's only reliable guide.
- Crocodiles: hold the deeper channels and take a boat's wake as an opening. Most of the danger on Adwe's water comes from what a rocking hull can't see over the side.
- Mosquito swarms: rise off the standing pools at dusk, thick enough to drive a camp back onto open water. The fever they leave behind outlasts the bites by a week.
