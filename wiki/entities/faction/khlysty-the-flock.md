---
title: Khlysty / the Flock
category: entities
tags: [shattered-sea, faction]
aliases: [Khlysty, the Flock, Khlysty - the Flock]
sources:
  - "Khlysty - the Flock.md"
  - "campaign-os:khlysty.md"
summary: Ecstatic Otel Karn tradition that treats flesh as spirit's vehicle and spreads blood anchors through noble houses.
provenance:
  extracted: 0.85
  inferred: 0.10
  ambiguous: 0.05
base_confidence: 0.38
lifecycle: proposed
lifecycle_changed: "2026-09-13"
tier: supporting
created: 2026-09-13T03:04:18Z
updated: 2026-09-17T22:00:08Z
type: faction
reveal: unrevealed
campaign: shattered-sea
visibility: dm
kind: organization
status: active
scope: realm
region: Otel Karn
base: [[otel-karn]]
relationships:
  - target: "[[otel-karn]]"
    type: related_to
  - target: "[[shepherd-grigori]]"
    type: related_to
  - target: "[[dravosi-crown]]"
    type: related_to
  - target: "[[tessarine-concordat]]"
    type: related_to
  - target: "[[chain-council]]"
    type: related_to
  - target: "[[pearl-of-souls]]"
    type: related_to
---
# Khlysty / the Flock

> [!narration] Public face
> The Dravosi Crown calls this tradition Khlysty, while its members call themselves the Flock. It is an ecstatic spiritual tradition rooted on Otel Karn, where its followers treat flesh as the vehicle of spirit and regard embodiment as a possible form of devotion.

````col
```col-md
flexGrow=2
===
## At a Glance

| | |
| --- | --- |
| **Nature** | Ecstatic spiritual tradition. |
| **Scope** | Realm. |
| **Base** | [[otel-karn]]. |
| **Leader** | [[shepherd-grigori]], the Korabl. |
| **Public purpose** | To pursue spiritual embodiment through the flesh. |
| **Signature method** | Quiet healing that leaves blood behind in another body. |
| **Current posture** | Acting. |

> **DM thesis:** The Flock turns healing and blood into a distributed survival system that can make whole noble houses into anchors.
```

```col-md
flexGrow=1
===
## At the Table

- **They want:** They want access to noble houses and bodies that can carry dormant blood-threads.
- **They offer:** They can offer quiet healing that leaves blood behind in another body.
- **They pressure with:** They make a single heir's survival into a problem spread across an entire bloodline.
- **They will not:** They will not treat flesh as separate from spiritual practice.
- **Their tell:** Members speak of the Flock rather than Khlysty, and frame excess or embodiment as devotion.
```
````

---

## Current State

- **Status quo.** The Flock is extending a tradition rooted on Otel Karn through quiet healing and dormant blood-threads in noble houses.
- **Recent change.** [[shepherd-grigori]] has carried the office of Korabl furthest and believes he authored the tradition's survival trick.
- **Pressure.** Grigori wants an army of anchors before blood-thinning or failing coherence can make the technique irrelevant.
- **Strength.** The Flock can hide its work inside ordinary healing and distribute its investment across multiple noble houses.
- **Vulnerability.** The tradition's origin is misunderstood by its current leader, and the Dravosi Crown has begun hunting the noble-house pattern.
- **Opportunity.** A court that accepts a quiet cure can become another anchor before it understands what entered its bloodline.
- **Doctrine.** The Turning joins dance, song, fasting, confession, and flagellation into a collective practice that seeks visions, healing, and altered states.

## Active Agenda

**Goal:** Establish an army of blood anchors across the [[dravosi-crown]], [[tessarine-concordat]], and [[chain-council]].

**Why:** A distributed network would make blood-thinning and failing coherence irrelevant.

**Next move:** Extend a quiet cure and dormant blood-thread into another noble house. ^[inferred]

**Needs:** A noble house, a vulnerable body, and access that does not trigger immediate Crown confirmation.

**Opposition:** [[dravosi-crown]], especially [[aleksander-malone]] when infiltration is confirmed.

**Next signal:** Another dying heir receives an unremarkable cure, while the house's blood begins to carry a dormant thread.

**Player opening:** The party can expose a cure, protect a targeted house, interrupt access to a patient, or force the Flock to reveal which houses it has already touched.

**If completed:** Multiple noble houses across the three named powers carry blood anchors, leaving no single heir or court as a point of failure.

## Assets

| Asset | Type | Location | What it enables | Condition |
| --- | --- | --- | --- | --- |
| [[pearl-of-souls]] | Magic | Unknown | A possible shortcut to transcendence. | Sought |
| Dormant blood-threads | Magic | Noble houses | Distributed survival across a bloodline. | Dormant |

## People & Structure

| Person / group | Role | Wants | Loyalty / fracture |
| --- | --- | --- | --- |
| [[shepherd-grigori]] | Korabl | To spread the survival trick and claim its authorship. | He does not know the technique predates him. |
| [[Otho]] | Source of the old teaching | No known current cult role. | Unaware that centuries have passed. |

### Chain of Action
**Who decides:** [[shepherd-grigori]] acts as Korabl.

**Who interprets those orders:** The Flock's local adherents; their structure is otherwise unknown.

**Who actually carries them out:** Members who perform the quiet cures and maintain blood-threaded houses.

**If leadership disappears:** The sect loses its designed coherence; whether existing anchors persist is unresolved.

No formal roster or hierarchy exists beyond Grigori. There is no second-in-command, and the sect is designed to lose coherence when he falls.

## Territory & Touchpoints

| Place | Presence | What they control or need here | Current pressure |
| --- | --- | --- | --- |
| [[otel-karn]] | Headquarters / Hidden | The old teaching and the tradition's original context. | Its history is misunderstood. |
| [[dravosi-crown]] | Hidden | Noble houses that can carry dormant threads. | [[aleksander-malone]] hunts confirmed infiltration. |
| [[tessarine-concordat]] | Hidden | Noble houses that can carry dormant threads. | Grigori wants the network to cross its courts. |
| [[chain-council]] | Hidden | Noble houses that can carry dormant threads. | Grigori wants the network to cross its courts. |

## Connections

| Faction / person | Stance | What the Flock wants from them | Leverage, debt, or friction |
| --- | --- | --- | --- |
| [[dravosi-crown]] | Hostile | Access to noble houses without confirmation. | The Crown calls the tradition Khlysty and reserves [[aleksander-malone]] for confirmed infiltration. |
| [[tessarine-concordat]] | Target | Access to noble houses. | The source gives no established Concordat response. |
| [[chain-council]] | Target | Access to noble houses. | The source gives no established Council response. |
| [[pearl-of-souls]] | Sought | A possible shortcut to transcendence. | The Pearl's current location is not established here. |

## Signals & Rumors

- **Visible sign.** A dying heir recovers after a quiet cure that leaves no obvious public explanation.
- **Rumor.** The Flock's teaching says that flesh can carry spirit rather than oppose it.
- **Direct signal.** The Crown assigns [[aleksander-malone]] when it confirms infiltration.
- **Quiet clue.** Several noble houses show the same dormant blood-thread pattern.

## Running the Faction

| Situation | Run it |
| --- | --- |
| **When encountered** | Flock members are performing a quiet cure or discussing how a body can preserve what the spirit cannot hold alone. |
| **When helped** | A successful cure becomes easier to conceal, and the Flock gains another body or house through which to extend its network. |
| **When opposed** | The Flock protects the pattern of its work first, abandoning a single patient or contact if that keeps the wider network hidden. |
| **When ignored** | Threads multiply across noble houses, and another court, dying heir, and quiet cure advance Grigori's army while the Crown argues over confirmation. |
| **When broken** | The source does not establish what survives if the Flock loses Grigori or its access to Otel Karn. |


## History

- **The old teaching on Otel Karn.** [[Otho]] is the archfey source of the teaching that became the Flock's spiritual foundation.

## Hidden Agenda

**Concealed motive:** When Grigori heals someone, he binds his phylactery into that person's life. The patient joins the Flock unaware, and their survival sustains him. Confirmed threads include a Tessarine heir in [[calven-and-calveno]] and a Dravosi merchant-lord's daughter.

**Concealed allegiance:** Grigori believes he authored the blood-anchor technique, but [[Otho]]'s old teaching on [[otel-karn]] predates him. The Turning and healings are genuinely warm; the horror is that Grigori has built an unkillable afterlife from the gratitude of desperate families.
