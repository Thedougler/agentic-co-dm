---
title: Sunkline
category: entities
tags: [shattered-sea, faction, sunkline, maw, salvage, tessarine]
sources:
  - "inbox/archive/FILED-2026-09-05-tessarine-concordat.md"
  - "legacy: /Users/nick/shattered-sea/wiki/shattered-sea/faction-sunkline-company.md"
  - "legacy: /Users/nick/shattered-sea/wiki/shattered-sea/situation-fathomrushs-split-harbor.md"
summary: "Tessarine-funded salvage company commercializing Mid-Works while hiding a forged mid-shelf trial certificate."
provenance:
  extracted: 0.95
  inferred: 0.05
  ambiguous: 0.0
base_confidence: 0.50
lifecycle: proposed
lifecycle_changed: "2026-09-13"
tier: supporting
created: 2026-09-13T20:25:00Z
updated: 2026-09-13T20:25:00Z
type: faction
reveal: unrevealed
campaign: shattered-sea
visibility: dm
status: active
kind: mercantile
relationships:
  - target: "[[Tessarine Concordat]]"
    type: related_to
  - target: "[[Voyvode]]"
    type: related_to
  - target: "[[Fathomrush]]"
    type: related_to
---
# Sunkline

> [!narration] Narration
> Dive lines, pressure rigs, and company stamps mark a Fathomrush compound that promises Mid-Works access. Crews take the depth; the paperwork stays ashore.

## Public face

Reach and commercialize the [[Mid-Works]] while keeping investors confident. Salvage orders, company law, pressure-rig trials, and paperwork are the tools.

## DM thesis

Keep a forged mid-shelf trial certificate from discovery long enough to preserve Tessarine money and salvage priority. When pressed, protect the false log and let crews absorb the risk.

## Current state

[[Tessarine Concordat]] credit funds the salvage pipeline. One [[Fathomrush]] compound holds dive lines, rigs, and the forged certificate. The false log is under a loose floorboard at the Last Berth; [[Petera Ngata]] keeps the complete death list.

## Active agenda

Prevent certificate discovery while the memorial wall fills. Losing the compound empties into [[Voyvode]]'s dock.

## Assets, people, places, relationships

- Chief engineer: [[Adriana Tarquine]]
- Store: [[Yevgenia Sorrel]]
- Rival: [[Voyvode]]
- Funder: [[Tessarine Concordat]]
- Places: [[Fathomrush]], [[Mid-Works]], Last Berth

## Connections

- [[Tessarine Concordat]]
- [[Voyvode]]
- [[Fathomrush]]
- [[Mid-Works]]
- [[Adriana Tarquine]]
- [[Yevgenia Sorrel]]
- [[Petera Ngata]]
